...
# ------------------------------------------------------------------
# ЧТО ЛЕЖИТ В АРХИВЕ — СКАЗАТЬ ВСЛУХ.
#
# Права стоят строгие (каталог 700, файл 600), но человек, который сам их
# и снимет, копируя архив на сетевой диск или в облако, об этом не
# задумается: слово «резервная копия» звучит как «копия писем». А внутри
# ещё и закрытый ключ TLS, весь infra/.env с паролями базы, Redis и
# админа, и ключи DKIM. Один такой архив на общей папке — это сервер
# целиком, включая возможность подписывать письма от имени домена.
#
# Молчать об этом нельзя: цена ошибки несопоставима со строчкой текста.
# ------------------------------------------------------------------
cat <<EOF

  В архиве лежат СЕКРЕТЫ: закрытый ключ TLS, ключи подписи DKIM и
  infra/.env с паролями базы, Redis и администратора — вместе со всей
  почтой. Права на файл выставлены строго (600, только root); держите
  его так же и не кладите в общие папки, облака и переписку.
