...
if [ -f "$ENV_FILE" ]; then
    ...
    chmod 600 "$ENV_FILE"
    ok "создан $ENV_FILE из образца"
fi

# ------------------------------------------------------------------
# СЕКРЕТЫ ЗАМЕНЯЮТСЯ СРАЗУ, А НЕ «ПОТОМ, КОГДА МАСТЕР ДОЙДЁТ ДО КОНЦА».
#
# Здесь и была дыра. Скрипт клал образец с заглушками (SESSION_SECRET,
# ADMIN_SESSION_SECRET, ключи шифрования, DOVECOT_MASTER_PASSWORD —
# пароль доступа ко ВСЕМ ящикам), а настоящие значения генерировала
# install/install.sh, которую мастер запускает последним шагом. Любой
# обрыв между этими двумя точками — закрыли вкладку, отвлеклись, не
# сошлись проверки, упёрлись в занятый порт — оставлял на машине рабочий
# файл с общеизвестными значениями из документации. Стек при этом
# поднимается и выглядит здоровым.
#
# Функция идемпотентна: уже сгенерированные секреты не трогает, так что
# повторный запуск скрипта ничего не ломает.
# ------------------------------------------------------------------
MADE="$(ensure_generated_secrets)"
if [ "$MADE" -gt 0 ]; then
    ok "заглушки заменены случайными секретами: $MADE"
else
    ok "секреты в $ENV_FILE уже настоящие — оставлены как есть"
fi
