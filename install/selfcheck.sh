...
# ------------------------------------------------------------------
# Порты берём ИЗ НАСТРОЕК, а не из литералов.
#
# load_env выше уже прочитал infra/.env, но проверки были написаны
# числами: на сервере с нестандартными портами самопроверка печатала
# «порт 25 никто не слушает» — при полностью исправной почте, — а тот
# порт, который сервер на самом деле публикует, не проверяла вовсе.
# ------------------------------------------------------------------
check_port "${SMTP_PORT:-25}"  "приём почты"
check_port "${SUBMISSION_PORT:-587}" "отправка, submission"
# 465 проверяется наравне с остальными, а не «если вдруг».
#
# Раньше здесь стояла успокаивающая строка «стек его пока не поддерживает».
# Она устарела молча: порт публикуется (install/compose.prod.yml), Postfix
# его слушает (master.cf), а автонастройка сама называет его почтовым
# клиентам (AUTOCONFIG_SUBMISSIONS_PORT). То есть Outlook и почта Apple,
# настроенные НАШЕЙ ЖЕ выдачей на 465, при сломанной публикации получают
# «соединение отклонено» — а самопроверка в этот момент отвечала «так и
# задумано» и выходила с кодом 0. Проверка, ради которой скрипт и
# существует, отсутствовала, а её место занимало утешение.
check_port "${SUBMISSIONS_PORT:-465}" "отправка, SMTPS (его выдаёт автонастройка Outlook и Apple)"
check_port "${IMAP_PORT:-143}" "IMAP + STARTTLS"
check_port "${IMAPS_PORT:-993}" "IMAPS"
check_port "${POP3_PORT:-110}" "POP3 + STARTTLS"
check_port "${POP3S_PORT:-995}" "POP3S"
check_port "${NGINX_HTTP_PORT:-80}"  "HTTP: автонастройка и продление сертификата"
check_port "${NGINX_HTTPS_PORT:-443}" "HTTPS: автонастройка (Outlook работает только по HTTPS)"
...
if [ -z "$DNS_JSON" ]; then
    ...
    while IFS=$'\t' read -r status rtype rname expected found comment; do
        ...
        case "$status" in
            ...
            # Сверять не с чем — не то же самое, что «всё хорошо». Так
            # бывает с DKIM до того, как rspamd выпустил ключ: раньше
            # такой случай печатался зелёным «опубликована и совпадает»
            # на чужой ключ, оставшийся от прежнего провайдера.
            unknown)  warn "$label — проверить пока нечем"
                      hint "${comment:-}" ;;
...
if [ -f "$ENV_FILE" ]; then
    ...
    if grep -q $'\r' "$ENV_FILE"; then
        ...
        # Подсказка нарочно НЕ через «sed -i». Он не правит файл, а
        # заменяет его новым, а infra/.env примонтирован в посредник
        # отдельным файлом — после такой замены посредник останется со
        # старым файлом-призраком и его правки перестанут доходить до
        # служб (см. проверку целости монтирования ниже).
        hint "починить: tr -d '\\r' < $ENV_FILE > /tmp/env.$$ && cat /tmp/env.$$ > $ENV_FILE && rm -f /tmp/env.$$"
    else
    ...
    # (infra/.env -> /env/.env), а такой bind-mount держит не имя, а
    # конкретный файл на диске. Достаточно один раз заменить файл, а не
    # переписать его содержимое, — «sed -i», «mv поверх», install(1),
    # распаковка архива, редактор с записью через временный файл, — и
    # контейнер останется со СТАРЫМ файлом, которого в каталоге уже нет.
    #
    # Ломается это тихо, и в этом вся беда. Панель сохраняет настройку,
    # посредник честно пишет её в свой файл-призрак, отвечает «готово»,
    # служба пересоздаётся — и поднимается с прежним окружением. Ни
    # ошибки, ни расхождения на экране: панель показывает сохранённое
    # значение, потому что оно и правда сохранено, только в базе.
    #
    # Сверяем номер файла (inode) снаружи и изнутри контейнера: у одного
    # и того же файла он один. Разошлись — монтирование порвано.
    if dc ps --status running --services 2>/dev/null | grep -qx 'service-agent'; then
        ENV_INODE_HOST="$(stat -c %i "$ENV_FILE" 2>/dev/null || echo '')"
        ENV_INODE_AGENT="$(dc exec -T service-agent stat -c %i /env/.env 2>/dev/null | tr -d '\r' | tr -d '\n')"
        if [ -z "$ENV_INODE_HOST" ] || [ -z "$ENV_INODE_AGENT" ]; then
            warn "не удалось сверить infra/.env с посредником (номер файла не прочитан)"
        elif [ "$ENV_INODE_HOST" = "$ENV_INODE_AGENT" ]; then
            ok "посредник видит тот же infra/.env, что и мы"
        else
            fail "посредник видит ЧУЖОЙ infra/.env: настройки из панели до служб не доходят"
            hint "файл заменили, а не переписали (sed -i, mv, install, распаковка архива)"
    ...
    case "$F2B_STATE" in
        *healthy*|running*)
            ...
            if ! command -v iptables >/dev/null 2>&1; then
                ...
                warn "цепочек бана в пакетном фильтре хоста ещё нет — их создаёт первый бан"
                # ------------------------------------------------------
                # ЗДЕСЬ СТОЯЛО НЕВЕРНОЕ ОБЪЯСНЕНИЕ.
                #
                # Было: «fail2ban заводит их при запуске камеры; если их
                # нет, служба не смогла изменить правила хоста». Это не
                # так, и проверено живьём: камеры подняты (четыре штуки),
                # а цепочек в DOCKER-USER нет — они появляются ТОЛЬКО
                # после первого настоящего бана и исчезают, когда
                # последний бан снят.
                #
                # Цена прежней формулировки: на любом свежем сервере, где
                # ещё никого не банили, самопроверка пугала «возможно,
                # банить не получится» и отправляла администратора искать
                # несуществующую поломку — NET_ADMIN, сеть хоста, вариант
                # iptables. Проверять надо не наличие цепочек, а сам бан,
                # что подсказка ниже и предлагает.
                #
                # Живой прогон на стенде, по шагам: до бана DOCKER-USER
                # пуста; после `fail2ban-client set mailtrue-dovecot banip
                # 203.0.113.7` в ней появляется переход в
                # f2b-mailtrue-dovecot, а внутри — REJECT на этот адрес;
                # после unban правило уходит, но САМА ЦЕПОЧКА И ПЕРЕХОД
                # ОСТАЮТСЯ (пустыми, с одним RETURN). То есть проверка
                # становится зелёной навсегда после первого же бана — и
                # это правильно: она отвечает на вопрос «может ли
                # fail2ban править правила хоста», а не «забанен ли
                # кто-то прямо сейчас».
                # ------------------------------------------------------
                hint "это нормально, пока подбора паролей не было: цепочку заводит"
                hint "первый забаненный адрес, и дальше она остаётся — уже пустой."
