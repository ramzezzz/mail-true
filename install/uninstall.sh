...
info "  • копии настроек рядом: .env.bak, .env.tmp, .env.before-…"
printf '\n'
info "Останутся на месте: образец $ENV_EXAMPLE (без него не переустановиться)"
info "и настройки других стеков этой машины ($ENV_FILE.<имя> и их копии)."
printf '\n'
...
# Поэтому идём по соседям «$ENV_FILE.*» и каталог сертификатов сносим целиком.
#
# ------------------------------------------------------------------
# НО «$ENV_FILE.*» — ЭТО НЕ ТОЛЬКО НАШИ КОПИИ
# ------------------------------------------------------------------
# Под ту же маску попадали два вида файлов, которые удалять нельзя вовсе:
#
#   infra/.env.example        — файл из репозитория, единственный образец
#                               настроек. Без него install/install.sh
#                               останавливается на «нет образца», то есть
#                               переустановиться из этого же каталога уже
#                               нельзя, а из распакованного архива — тем
#                               более: там взять его неоткуда.
#   infra/.env.<имя стека>    — настройки ДРУГОГО стека на этой машине
#   infra/.env.<имя>.before-… (стенд рядом с боевым). Снятие одного стека
#                               уносило у соседа .env со всеми секретами:
#                               паролем базы, ключами шифрования, паролем
#                               служебного доступа ко всем ящикам. Живой
#                               сервер при этом продолжает работать —
#                               ровно до первого перезапуска.
#
# Поэтому удаляем не «всё, что начинается с .env.», а только те хвосты,
# которые делаем сами: .bak (install.sh), .tmp и .before-* (change-domain.sh,
# restore.sh). Разбор живёт в env_copies_purge (lib/common.sh) — там же его
# проверяет install/selftest.sh, настоящими файлами, а не чтением исходника.
# ------------------------------------------------------------------
for f in "$ENV_FILE" "$STATE_FILE"; do
    if [ -f "$f" ]; then rm -f "$f"; ok "удалён $f"; fi
done
PURGED="$(env_copies_purge)"
LEFTOVERS="${PURGED%% *}"
FOREIGN="${PURGED##* }"
if [ "$LEFTOVERS" -gt 0 ]; then
    ok "удалены копии настроек рядом с ним: $LEFTOVERS шт. (.bak, .tmp, .before-*)"
fi
if [ "$FOREIGN" -gt 0 ]; then
    info "оставлено рядом файлов, которые нам не принадлежат: $FOREIGN шт."
    hint "это образец .env.example и настройки других стеков — их удаляют вручную"
fi
if [ -d "$CERT_DIR" ]; then
