...
# Ответ пришёл извне — из файла ответов веб-мастера или из командной
# строки. Это осознанное решение человека, и ниже оно должно быть сильнее
# любых умолчаний; отличить его от «просто нажали Enter» больше негде.
CLAMAV_EXPLICIT=0
[ -n "$CLAMAV_CHOICE" ] && CLAMAV_EXPLICIT=1
...
#
# ------------------------------------------------------------------
# ПЕРЕСПРАШИВАТЬ МОЖНО ТОЛЬКО ЖИВОГО ЧЕЛОВЕКА
# ------------------------------------------------------------------
# Здесь стоял безусловный confirm, и это тихо отменяло уже данный ответ.
# confirm в неинтерактивном режиме (MT_NONINTERACTIVE=1 — его ставит ветка
# --answers, которой пользуется мастер в браузере) ВСЕГДА отвечает «нет»:
# спрашивать некого. То есть галочка «Антивирус» в мастере превращалась в
# CLAMAV_ENABLED=false ровно на тех машинах с 2 ГБ, о которых и написано
# предупреждение, — и человек об этом не узнавал: ни ошибки, ни отметки в
# панели, только одна строка info в длинном журнале установки.
#
# Тот же промах бил и по обновлению: на сервере с уже включённым
# антивирусом и малой памятью автоматический запуск install.sh молча его
# выключал, хотя выключить никто не просил.
#
# Поэтому: предупреждаем всегда, а отменять чужой ответ — только если есть
# кому ответить и ответ не был задан явно.
# ------------------------------------------------------------------
RAM_MB="$(ram_total_mb)"
if [ "$CLAMAV_CHOICE" = "yes" ] && [ "$RAM_MB" -gt 0 ] && [ "$RAM_MB" -lt "$MT_MIN_RAM_CLAMAV_MB" ]; then
    ...
    if [ "$CLAMAV_EXPLICIT" = "1" ] || [ "$MT_NONINTERACTIVE" = "1" ]; then
        warn "антивирус всё равно включаю: об этом попросили явно, а переспросить некого"
        hint "стеку может не хватить памяти — следите за ним или выключите антивирус:"
        hint "  MAILTRUE_CLAMAV=no bash install/install.sh"
    elif ! confirm "Всё равно включить антивирус?"; then
        CLAMAV_CHOICE=no
...
#
# Сама env_port живёт в install/lib/common.sh — там же, где env_set и
# env_ensure, и там же её разбирает самопроверка. Пока копия функции
# лежала в selftest.sh рядом с пометкой «ровно та функция, что стоит в
# install.sh», проверка сторожила копию: расхождение между ними и было бы
