...
for mig in "$INFRA_DIR"/postgres/migrations/*.sql; do
    ...
    if dc exec -T postgres psql -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -q \
            < "$mig" >/dev/null 2>"$MIG_ERR"; then
        ok "миграция $mig_name применена"
    else
        fail "миграция $mig_name не применилась"
        # Сама ошибка psql раньше уходила в /dev/null вместе с выводом, и
        # человеку оставалось только гадать. Причина почти всегда написана
        # в первой же строке — печатаем её, а не предлагаем найти самому.
        sed -n '1,5p' "$MIG_ERR" | sed 's/^/           /'
        MIG_FAILED="$MIG_FAILED $mig_name"
    fi
done
rm -f "$MIG_ERR"
