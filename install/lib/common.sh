...
# 465 (SMTPS) в списке нет намеренно: Postfix его слушает ВНУТРИ контейнера
# (служба submissions в infra/postfix/conf/master.cf), но боевое
# переопределение портов (install/compose.prod.yml) наружу его не выпускает.
# Проверять занятость порта, который мы не займём, значит отказывать в
# установке из-за чужой службы, которая нам не мешает.
# Подробности и как открыть — docs/install.md, раздел «Порты».
