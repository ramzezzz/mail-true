...
#
# ПРО ПЕРВЫЙ ЗАПУСК НА УЖЕ РАБОТАЮЩЕМ СЕРВЕРЕ. Журнал там пуст, а схема
# накатана. Отметить весь каталог применённым «задним числом» было бы
# неверно: сервер может стоять на 0015, а в каталоге лежать 0021, и такая
# отметка тихо пропустила бы шесть настоящих миграций. Узнать версию
# неоткуда — её негде было хранить. Поэтому первый запуск ведёт себя ровно
# как все запуски до появления журнала: применяет каталог целиком (файлы
# идемпотентны) и записывает результат. Со второго запуска — только новое.
#
...
apply_migrations() {
    ...
    local mig mig_name sum ver name known errfile
    local registry="$INFRA_DIR/postgres/migrations/0000_schema_migrations.sql"
    ...
    for mig in "$INFRA_DIR"/postgres/migrations/*.sql; do
        ...
        if psql_migrate < "$mig" >/dev/null 2>"$errfile"; then
            ...
            # ON CONFLICT нужен для случая «файл поправили после выпуска»:
            # строка уже есть, но с прежней суммой.
            #
            # Запрос подаётся ЧЕРЕЗ stdin, а не ключом -c, и это не стиль.
            # В аргументе -c psql подстановку своих переменных (:'fn') НЕ
            # делает — строка уходит на сервер как есть, и Postgres отвечает
            # «syntax error at or near ":"». Поймано живым прогоном: схема
            # накатывалась, а журнал оставался пустым, то есть весь смысл
            # затеи пропадал молча. Подстановка работает только для потока
            # ввода и файлов, поэтому запрос идёт сюда.
            if printf '%s\n' \
                 ...
                 psql_migrate \
                    ...
                    -v ck="$sum" -v who="$who" >/dev/null 2>"$errfile"; then
