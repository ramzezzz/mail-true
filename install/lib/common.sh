...
# ------------------------------------------------------------------
# Ключи infra/.env, ПРИВЯЗАННЫЕ К МАШИНЕ, а не к установке.
#
...
#
# Восстановление кладёт .env из копии целиком, и на ДРУГОЙ машине этот
# адрес не существует. Docker отказывается публиковать порт на чужом
# адресе — «cannot assign requested address», — и от этого не поднимается
# ВЕСЬ стек, а не одна служба. То есть перенос на новый сервер (главный
# сценарий, ради которого копию и снимают) заканчивался мёртвым стеком, а
# сообщение говорило про порт 8081, к аварии отношения не имевший.
#
# Поэтому такие ключи после восстановления сверяются с машиной: см.
# fix_machine_bound_env в install/restore.sh.
# ------------------------------------------------------------------
MT_MACHINE_BOUND_ENV_KEYS=(ADMIN_LOCAL_BIND)
...
# ------------------------------------------------------------------
# ВСЕ секреты infra/.env — ОДНИМ списком: «ключ:длина».
#
# Список жил внутри install/install.sh, а самопроверка перечисляла свои
# три ключа отдельной строкой (POSTGRES|REDIS|RSPAMD). Расхождение
# кончилось тем, что заглушки SESSION_SECRET, ADMIN_SESSION_SECRET,
# AI_ENCRYPTION_KEY, EXTERNAL_ACCOUNTS_KEY и DOVECOT_MASTER_PASSWORD
# оставались на боевом сервере, а самопроверка печатала «пароли в
# infra/.env не из примера» — то есть подтверждала обратное.
#
# Теперь список один: по нему секреты генерируются (ensure_generated_secrets)
# и по нему же проверяются (install/selfcheck.sh). Добавили ключ — он сразу
# и генерируется, и проверяется; забыть половину больше негде.
#
# QUEUE_AGENT_TOKEN и SERVICE_AGENT_TOKEN стоят здесь не для красоты. В
# infra/.env.example они ПУСТЫЕ, а пустое значение означает «посредника к
# очереди Postfix не запускать вовсе» (infra/postfix/entrypoint.sh) и
# «раздел „Очередь“ в панели недоступен».
# ------------------------------------------------------------------
MT_GENERATED_SECRETS=(
...
# ------------------------------------------------------------------
# env_is_from_example <ключ> — значение в infra/.env дословно совпадает
# с тем, что стоит в infra/.env.example.
#
# ЗАЧЕМ. Установщику постоянно нужно решить: «это значение поставил
# человек — не трогаем» или «это значение поставил образец — заменяем
# боевым». Различить их неоткуда, кроме как сверив с образцом: свежий
# infra/.env — его дословная копия (см. install.sh, шаг «Секреты и
# конфигурация»), поэтому в нём нет ни одной строки, которую кто-то
# осознанно набрал.
#
# ЧТО БЫЛО. Без этой развилки «непустое значит человеческое» — и
# установка с нуля уезжала с настройками машины разработчика. Образец
# держит NGINX_HTTP_PORT=8080 и NGINX_HTTPS_PORT=8443 (на dev-машине 80
# и 443 обычно заняты), env_ensure их не трогал, и боевой сервер после
# полностью зелёной установки не отвечал ни на одном адресе, который сам
# же напечатал в конце. То же самое с REDIS_PORT=6380.
#
# Ключа нет в образце (или он там пустой) — сверять не с чем, отвечаем
# «нет»: значение точно не оттуда.
# ------------------------------------------------------------------
env_is_from_example() {
    local key="$1" sample
    [ -f "$ENV_EXAMPLE" ] || return 1
    sample="$(env_get "$key" "$ENV_EXAMPLE")"
    [ -n "$sample" ] || return 1
    [ "$(env_get "$key")" = "$sample" ]
}

# ------------------------------------------------------------------
# env_port <ключ> <заданное явно> <умолчание> — публикуемый порт.
#
# Правило одно на все двенадцать портов, и каждая его часть оплачена
# отдельной поломкой у человека, который ставил сервер.
#
# 1. ЗАДАННОЕ ЯВНО (MAILTRUE_*_PORT, ими пользуется веб-установщик на шаге
#    «Порты») пишется всегда: это осознанный ввод, спорить не о чем.
#
# 2. ПУСТО ИЛИ УЖЕ РАВНО УМОЛЧАНИЮ — пишем умолчание. Стандартные номера
#    менять на боевом сервере не нужно: чужие почтовые серверы стучатся в
#    25-й, почтовые программы — в 587/993, и наша же автонастройка
#    называет им именно эти номера.
#
# 3. ЗНАЧЕНИЕ ИЗ ОБРАЗЦА НА ЕЩЁ НЕ УСТАНОВЛЕННОМ СЕРВЕРЕ заменяется
#    умолчанием — вслух.
#
#    Свежий infra/.env — дословная копия infra/.env.example, а образец
#    рассчитан ещё и на dev-машину, где 80 и 443 обычно заняты:
#    NGINX_HTTP_PORT=8080, NGINX_HTTPS_PORT=8443, REDIS_PORT=6380.
#    Пустыми эти строки не были никогда, поэтому «ставим, только если
#    пусто» до них не доходило, и install/compose.prod.yml публиковал веб
#    на 8080/8443 — подстановка «${NGINX_HTTP_PORT:-80}» своего умолчания
#    не видит, значение-то есть.
#
#    Установка при этом проходила ЦЕЛИКОМ ЗЕЛЁНОЙ и не могла не пройти:
#    nginx проверяет себя изнутри контейнера, на 80-м порту контейнера;
#    предпусковая проверка докладывала «порт 80 свободен» — он и правда
#    был свободен, потому что его никто не занимал; certbot выпускал
#    сертификат через --standalone, своим слушателем. Человек читал в
#    конце «Почта: https://mail.example.ru» и получал отказ соединения.
#    Вместе с ним не работала автонастройка Outlook и Apple Mail.
#
#    УСЛОВИЯ ДВА, И ВТОРОЕ ОБЯЗАТЕЛЬНО. «Совпадает с образцом» само по
#    себе ещё не значит «поставил образец»: мастер в браузере на шаге
#    «Порты» вполне законно предлагает те же 8080/8443, когда на машине
#    уже живёт другой стек. Поэтому спрашиваем ещё и отметку
#    INSTALL_COMPLETED_AT: пока её нет, установка ни разу не доходила до
#    конца и набирать порты руками было негде и незачем. Как только она
#    появилась, каждое число в файле — это порт РАБОТАЮЩЕГО сервера, и
#    трогать его нельзя ни при каком совпадении.
#
# 4. ВСЁ ОСТАЛЬНОЕ НЕ ТРОГАЕМ: значение поставил человек. Раньше здесь
#    стоял безусловный env_set, и второй запуск установщика — обновление,
#    починка, переустановка поверх — молча возвращал все порты к
#    стандартным. Стенд, живший на 2525-м, после обновления пытался занять
#    25-й, не поднимался вовсе, и виноватым выглядел docker: «port is
#    already allocated».
# ------------------------------------------------------------------
env_port() {
    local key="$1" explicit="$2" fallback="$3" current
    if [ -n "$explicit" ]; then
        env_set "$key" "$explicit"
        return 0
    fi
    current="$(env_get "$key")"
    if [ -z "$current" ] || [ "$current" = "$fallback" ]; then
        # env_ensure возвращает 1, когда значение уже было, — под set -e
        # это оборвало бы установку.
        env_ensure "$key" "$fallback" || true
        return 0
    fi
    if [ -z "$(install_mark_env)" ] && env_is_from_example "$key"; then
        env_set "$key" "$fallback"
        info "$key=$current — это значение из образца, а не ваше; ставлю боевое $fallback"
        hint "нужен именно $current — задайте его явно: MAILTRUE_${key}=$current"
        return 0
    fi
    return 0
}
...
# ------------------------------------------------------------------
# env_copies_purge [файл настроек] — снести НАШИ копии .env при полном
# удалении. Печатает два числа через пробел: «удалено оставлено».
#
# ЗАЧЕМ. В .env лежит всё сразу: пароль базы, ключи шифрования сохранённых
# паролей чужих ящиков, секрет сессий, пароль служебного доступа ко ВСЕМ
# ящикам. Скрипты обслуживания делают его полные копии (.bak, .tmp,
# .before-domain-change-<id>), и после «удалено безвозвратно» они
# оставались лежать на машине, которую отдают, продают или передают
# другому администратору.
#
# ЧТО БЫЛО. Копии сносили маской «<файл>.*» — и она забирала лишнее:
#
#   infra/.env.example      файл из репозитория, единственный образец
#                           настроек. Без него install/install.sh
#                           останавливается на «нет образца»: переустановка
#                           из того же каталога становится невозможной, а из
#                           распакованного архива — тем более, взять его
#                           там неоткуда.
#   infra/.env.<стек>       настройки ДРУГОГО стека на этой машине (стенд
#   infra/.env.<стек>.…     рядом с боевым). Снятие одного стека уносило у
#                           соседа .env со всеми секретами. Сосед при этом
#                           продолжает работать — ровно до перезапуска.
#
# Поэтому сверяем ХВОСТ имени, а не начало: наши — только .bak, .tmp и
# .before-*. Всё прочее считаем чужим и пересчитываем, чтобы «удалено
# безвозвратно» не звучало шире, чем сделано.
# ------------------------------------------------------------------
env_copies_purge() {
    local base="${1:-$ENV_FILE}" name f removed=0 kept=0
    # Сравниваем по имени файла, а не по пути: каталог установки может
    # называться как угодно, в том числе содержать точку.
    name="${base##*/}"
    for f in "$base".*; do
        [ -e "$f" ] || continue
        case "${f##*/}" in
            "$name".bak|"$name".tmp|"$name".before-*)
                rm -f "$f"; removed=$((removed + 1)) ;;
            *)
                kept=$((kept + 1)) ;;
        esac
    done
    printf '%s %s' "$removed" "$kept"
}

# ------------------------------------------------------------------
# ensure_generated_secrets — заменить заглушки настоящими секретами.
#
# Печатает, сколько ключей сгенерировано. Идемпотентна: уже сгенерированный
# секрет не меняется — иначе пароль в .env разъехался бы с паролем внутри
# уже созданной базы.
#
# Вызывается ДВАЖДЫ и из разных мест, и это принципиально. Установка из
# браузера (install/web-install.sh) кладёт infra/.env из образца и поднимает
# мастер; настоящие секреты генерировала только install/install.sh, которую
# мастер запускает В КОНЦЕ. То есть любой обрыв мастера — закрыли вкладку,
# отвлеклись, не сошлись проверки — оставлял на машине БОЕВОЙ файл с
# общеизвестными значениями из документации: SESSION_SECRET, ключи
# шифрования, пароль служебного доступа ко всем ящикам. Стек при этом
# поднимается и работает.
#
# Поэтому образец превращается в настоящий файл сразу, а не «потом».
# ------------------------------------------------------------------
ensure_generated_secrets() {
    local pair key len current made=0
    for pair in "${MT_GENERATED_SECRETS[@]}"; do
        key="${pair%%:*}"; len="${pair##*:}"
        current="$(env_get "$key")"
        # Пусто или заглушка из примера — генерируем. is_placeholder_password
        # знает оба написания: и «change-me-…», и «смените-этот-пароль».
        if [ -z "$current" ] || is_placeholder_password "$current"; then
...
# ------------------------------------------------------------------
# dc_exec_secret <сервис> <ИМЯ> <значение> <скрипт для sh> [аргументы...]
#
# Выполняет скрипт внутри контейнера, передав секрет ЧЕРЕЗ ВВОД, а не
# аргументом командной строки.
#
# Зачем. Строка запуска любого процесса лежит в /proc/<pid>/cmdline и
# читается ЛЮБЫМ пользователем машины — права на файл 444. Поэтому
# «docker compose exec -e PGPASSWORD=… postgres psql …» показывает пароль
# базы каждому, у кого есть учётная запись на сервере, ровно на то время,
# пока идёт команда. Скрипты обслуживания запускают от root и обычно
# тогда, когда на сервере что-то чинят — то есть когда там сидят люди.
#
# Окружение процесса (/proc/<pid>/environ) так не читается: только своим
# владельцем и root. Поэтому секрет уходит в контейнер через stdin, там
# кладётся в переменную окружения и оттуда достаётся программой (psql
# читает PGPASSWORD, redis-cli — REDISCLI_AUTH).
#
# Внутри скрипта секрет доступен как $ИМЯ, дополнительные аргументы — как
# "$@". Кавычки в скрипте одинарные: он раскрывается ВНУТРИ контейнера.
# ------------------------------------------------------------------
dc_exec_secret() {
    local service="$1" name="$2" value="$3" script="$4"
    shift 4
    printf '%s\n' "$value" | dc exec -T "$service" sh -c '
        IFS= read -r mt_secret
        export "$1=$mt_secret"
        unset mt_secret
        mt_script="$2"
        shift 2
...
_mig_apply_file() {
    ...
    # Файл уже выполнил САМ Postgres при первичной инициализации базы.
    #
    # Каталог миграций примонтирован в /docker-entrypoint-initdb.d, поэтому на
    # новом сервере всё содержимое каталога отрабатывает ещё до первого запуска
    # установщика. Раньше журнал знал об этом только от базовой схемы, и
    # остальные миграции применялись ВТОРОЙ раз — а среди них уже есть DELETE
    # по пользовательским данным. Теперь каждая пишет о себе строку с суммой
    # 'initdb' (настоящую внутри SQL не вычислить), а мы её узнаём и заменяем
    # на настоящую: дальше файл сверяется как обычно, по содержимому.
    if printf '%s\n' "$known" | grep -qxF "$mig_name|initdb"; then
        _mig_record "$mig_name" "$sum" "$who" || true
        MT_MIG_SKIPPED=$((MT_MIG_SKIPPED + 1))
        return 0
    fi
...
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# host_has_ip <адрес> — этот адрес и правда есть на этой машине?
#
# Нужен восстановлению: адрес из чужого .env docker опубликовать не может
# и валит весь стек. 127.0.0.1 считаем своим всегда — петля есть везде,
# а `ip` на машине может и не оказаться.
# ------------------------------------------------------------------
host_has_ip() {
    local addr="$1"
    [ -n "$addr" ] || return 1
    ...
    have ip || return 0   # проверить нечем — не мешаем работать
    ip -o addr show 2>/dev/null | awk '{ split($4, a, "/"); print a[1] }' |
        grep -qxF "$addr"
}

# ------------------------------------------------------------------
# ram_total_mb — сколько на машине памяти, в мегабайтах (0 — неизвестно).
#
# Функция общая, и это не украшательство. Значение читалось в двух местах
# по-своему: предполётная проверка считала его сама, а install.sh
# обращался к переменной RAM_MB, которую НИКТО НЕ ЗАПОЛНЯЛ. Под `set -u`
# это не «ноль», а обрыв: «RAM_MB: unbound variable» — и установка
# заканчивалась ровно на ответе «да» на вопрос про антивирус. И в консоли,
# и через браузер: включить ClamAV было невозможно в принципе.
# ------------------------------------------------------------------
ram_total_mb() {
    local mb=0
...
# ------------------------------------------------------------------
# cert_reachable_names <имя сервера> <домен> — какие имена уже ведут сюда.
#
# Заполняет массив MT_CERT_NAMES и возвращает 1, если не ведёт ни одно.
# Let's Encrypt проверяет владение HTTP-запросом по каждому имени: из-за
# одного неопубликованного имени он отказывает во ВСЁМ выпуске целиком,
# поэтому в заявку идут только те, чья A-запись указывает на нас.
#
# Функция общая для install.sh (первый выпуск) и renew-certs.sh --force
# (выпуск, когда сертификата ещё нет). Пока её не было, «--force» умел
# только продлевать уже существующий: `certbot renew --cert-name mailtrue`
# на сервере без этого сертификата отвечает «no certificate found with
# name mailtrue» и выходит с ошибкой. А отсылали на этот ключ из пяти
# мест — в том числе из совета «выпуск не удался, повторите так».
# ------------------------------------------------------------------
MT_CERT_NAMES=()
cert_reachable_names() {
    local mail_host="$1" domain="$2"
