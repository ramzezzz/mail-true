...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        # Проверка значения — ниже, после разбора всех ключей.
...
# ------------------------------------------------------------------
# ЗНАЧЕНИЕ --only ПРОВЕРЯЕТСЯ, А НЕ ПРИНИМАЕТСЯ НА ВЕРУ
# ------------------------------------------------------------------
# Здесь принималось ЛЮБОЕ слово. Дальше оно попадало в want(), который
# сравнивает его с именами разделов, — и опечатка вроде «--only base»
# или «--only mails» просто не совпадала ни с одним. Восстановление
# честно проходило все шаги, не восстановив НИЧЕГО, и заканчивалось
# бодрым отчётом.
#
# Хуже места для тихой ошибки не придумать: восстановление запускают
# один раз, в аварии, и «готово» на экране означает, что дальше человек
# ищет пропавшие письма где угодно, только не здесь.
case "$ONLY" in
    ''|db|mail|config) ;;
    *) die "--only принимает db, mail или config, а получено «$ONLY»" ;;
esac
...
if want config && [ -d "$WORK/config" ]; then
    ...
    if [ -f "$WORK/config/.env" ]; then
        ...
        # ------------------------------------------------------------------
        # ФАЙЛ ПЕРЕЗАПИСЫВАЕТСЯ НА МЕСТЕ, А НЕ ЗАМЕНЯЕТСЯ ЦЕЛИКОМ
        # ------------------------------------------------------------------
        # Здесь стоял install(1), и он ЗАМЕНЯЛ файл: снимал прежний и
        # создавал новый. Для обычного файла разницы нет, для этого —
        # есть, и цена высокая.
        #
        # infra/.env примонтирован в посредник ОТДЕЛЬНЫМ ФАЙЛОМ
        # (infra/.env -> /env/.env). Такой bind-mount держит не имя, а
        # конкретный файл на диске. Заменили файл новым — контейнер
        # остаётся со СТАРЫМ, которого в каталоге уже нет: тот же путь,
        # разное содержимое.
        #
        # Дальше всё ломается тихо. Панель сохраняет настройку, посредник
        # честно пишет её в свой файл-призрак, отвечает «готово», служба
        # пересоздаётся — и поднимается с прежним окружением. Ни ошибки,
        # ни расхождения на экране: панель показывает сохранённое
        # значение, потому что оно и правда сохранено — в базе.
        #
        # Поэтому пишем ПОВЕРХ существующего файла: перенаправление в него
        # обрезает содержимое, но сам файл (и его inode) остаётся тем же —
        # ровно то же правило, что и в env_set (install/lib/common.sh).
        cat "$WORK/config/.env" > "$ENV_FILE"
        chmod 600 "$ENV_FILE"
        ...
        # ------------------------------------------------------------------
        # АДРЕСА ИЗ ЧУЖОЙ МАШИНЫ ЗДЕСЬ НЕ РАБОТАЮТ.
        #
        # ADMIN_LOCAL_BIND — адрес, на котором публикуется резервный вход в
        # панель; у каждой машины он свой (обычно её адрес в локальной сети,
        # например 192.168.1.40). Файл .env берётся из копии целиком, и на
        # НОВОМ сервере этого адреса не существует.
        #
        # Дальше не «одна служба не поднялась», а мёртвый стек: docker
        # отказывается публиковать порт на чужом адресе — «cannot assign
        # requested address» — и `docker compose up` валится целиком.
        # Восстановление честно печатало «стек не поднимается после
        # восстановления», и человек в разгар аварии искал причину в базе,
        # томах и образах. А речь про резервный вход на порту 8081, к
        # восстановлению отношения не имеющий.
        #
        # Именно перенос на другую машину — главный повод снимать копию.
        # Поэтому такие ключи сверяются с машиной и, если адрес не её,
        # заменяются на здешний (или 127.0.0.1). Об этом говорим вслух:
        # человек должен знать, что резервный вход сменил адрес.
        # ------------------------------------------------------------------
        for key in "${MT_MACHINE_BOUND_ENV_KEYS[@]}"; do
            bound_value="$(env_get "$key")"
            [ -n "$bound_value" ] || continue
            if host_has_ip "$bound_value"; then continue; fi
            new_value="$(private_ip)"
            new_value="${new_value:-127.0.0.1}"
...
# Пароль уходит в контейнер ЧЕРЕЗ ВВОД, а не ключом «-e PGPASSWORD=…»:
# строка запуска docker видна в /proc любому пользователю машины, а
# окружение процесса — нет. Подробнее в dc_exec_secret (lib/common.sh).
if dc_exec_secret postgres PGPASSWORD "$POSTGRES_PASSWORD" \
        'psql -h postgres -U "$1" -d "$2" -qtA -c "SELECT 1"' \
        "$POSTGRES_USER" "$POSTGRES_DB" >/dev/null 2>&1; then
