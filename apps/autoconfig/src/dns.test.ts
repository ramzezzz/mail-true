...
test('dkim: склейка многострочного значения из файла rspamd', () => {
  ...
  assert.ok(value);
  assert.match(value, /^v=DKIM1; k=rsa; p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA0jawqKcBkxLpXM4E6s9CrFafL2JX2hwOyIkAQAB$/);
});
...
test('dns-records: полный набор записей', () => {
  ...
  assert.match(find('_dmarc', 'TXT')?.value ?? '', /^v=DMARC1; p=quarantine; rua=mailto:postmaster@mail\.local/);
