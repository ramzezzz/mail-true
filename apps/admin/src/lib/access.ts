...
export const NAV_ITEMS: readonly NavItem[] = [
  ...
  // Логотипы доменов отправителей. Право то же, что у оформления входа, и
  // по той же причине: и там и здесь речь о картинках, которые продукт
  // показывает от своего имени. Изменение прячется отдельно, по
  // branding.write, как и у логотипа входа.
  { to: '/sender-logos', title: 'Логотипы доменов', requires: ['branding.read'] },
  // Резервные копии НАСТРОЕК (письма — install/backup.sh). Заглушкой
  // раздел больше не является. Пункт виден только тем, кто может
  // скачать копию или восстановить её: внутри файла хэши паролей.
  { to: '/backups', title: 'Резервные копии', requires: ['backup.export', 'backup.restore'] },
  // Перенос почты с чужого сервера. Пункт виден и «только чтению»: вопрос
  // «доехала ли почта Иванова» задают дежурному, и ответ на этом экране.
  { to: '/migrate', title: 'Перенос почты', requires: ['migration.read'] },
  /*
   * Антиспам. Заглушкой раздел больше не является: статистика и списки
   * читаются у контроллера rspamd, правки списков он же и записывает.
   * Пункт виден и «только чтению» — вопрос «почему письмо ушло в спам»
   * задают дежурному; кнопки изменения прячутся отдельно, по domains.write.
   */
  { to: '/spam', title: 'Спам', requires: ['overview.read'] },
  /*
