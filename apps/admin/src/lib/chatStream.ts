...
/**
 * Предел одной реплики, который применяет сервер.
 *
 * ------------------------------------------------------------------
 * ЗАЧЕМ ОН ЗДЕСЬ, ЕСЛИ ЕСТЬ НА СЕРВЕРЕ
 * ------------------------------------------------------------------
 * История разговора уезжает на сервер ЦЕЛИКОМ с каждым вопросом, и схема
 * применяется ко всей истории. Значит одна слишком длинная реплика ломает
 * не текущий вопрос, а ВЕСЬ дальнейший разговор: она остаётся в истории,
 * и каждый следующий вопрос отбивается той же ошибкой. Починка была одна —
 * закрыть окно и потерять разговор целиком.
 *
 * Поэтому предел виден и здесь: поле ввода не даёт набрать больше, а
 * ответы помощника обрезаются перед отправкой. На экране при этом
 * остаётся полный текст — обрезка живёт только в том, что уходит наружу.
 */
export const CHAT_TURN_MAX_CHARS = 4000;

/** Обрезает ответы помощника до предела. Вопросы человека не трогает. */
export function trimHistoryForServer(turns: readonly ChatTurn[]): ChatTurn[] {
