/**
 * Расчёт контраста по WCAG 2.x.
 *
 * Свой файл, а не импорт из apps/web: приложения кодом не делятся, а формула
 * относительной яркости — три строки, ради которых тянуть чужой модуль не
 * стоит. Пользуются им проверки палитры входа в панель.
 */
...
/** Относительная яркость sRGB-цвета (WCAG 2.x, «relative luminance»). */
 ...
 * Кружки значков на глобусе полупрозрачны, и без сведения слоёв контраст
 * значка считался бы не по тому цвету, который человек видит.
 */
