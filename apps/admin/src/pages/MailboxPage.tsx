    ...
    return (
      <>
        ...
        <Notice tone="error">
          ...
          <code className="mt-mono">DOVECOT_MASTER_PASSWORD</code> в{' '}
          <code className="mt-mono">infra/.env</code> и в окружении API, затем перезапустите
          dovecot. Подменять пароль пользователя админка не будет — это оставило бы владельца без
          доступа.
        </Notice>
      </>
    );
  ...
  if (!active) {
    return (
      <>
        <PageTitle title="Ящик пользователя" />
        <Notice tone="info">
          Сейчас вы не в чужом ящике. Войти в него можно кнопкой «Войти в ящик» в строке нужного
          ящика — в разделе «Пользователи».
        </Notice>
        <Panel>
          <Link to="/users">
  ...
  return (
    <>
      ...
      <Notice tone="error">
        ...
        <br />
        Причина: {active.reason}. Начало сеанса: {formatDateTime(active.startedAt)}. Отправка писем
        запрещена.{' '}
