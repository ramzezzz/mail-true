...
function UsersSection({ hours, poll }: { hours: number; poll: number | false }) {
  ...
  /*
   * Молчавшие — число С СЕРВЕРА, а не подсчёт по показанным строкам.
   *
   * Здесь стоял фильтр по users.data.items, то есть по запрошенной
   * странице в 25 строк, отсортированной сервером по трафику по
   * убыванию. Молчащие ящики стоят в самом хвосте этого порядка и в
   * первые 25 строк не попадают никогда: на сервере со 143 ящиками
   * подпись честным тоном сообщала «Молчали за период: 0» — то есть
   * ровно противоположное правде, и выглядела при этом измеренной.
   */
  const silent = users.data?.silent ?? 0;
