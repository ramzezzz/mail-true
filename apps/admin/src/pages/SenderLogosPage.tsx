...
export function SenderLogosPage() {
  ...
  const list = useQuery({
    ...
    queryFn: () => api.senderLogos({ ...(search ? { q: search } : {}), limit: PAGE_SIZE, offset }),
  });
  ...
  return (
    <>
      ...
      <Panel title="Как это работает">
        <p style={MUTED}>
          Логотип домена сервер ищет сам: сначала в записи BIMI, которую владелец домена публикует в
          DNS специально для почты, затем по значку сайта. Картинки забирает и хранит сервер —
          браузеры пользователей к чужим сайтам не обращаются. Логотип показывается только у писем,
          чья подлинность подтверждена проверкой DKIM/DMARC: рядом с поддельным отправителем он
          опаснее, чем его отсутствие.
        </p>
        <p style={MUTED}>
          Здесь можно вмешаться. Порядок такой: <b>запрет</b> сильнее всего, <b>своя картинка</b>{' '}
          сильнее найденной, найденная действует, когда нет ни того, ни другого. Свои картинки
          хранятся в базе и попадают в резервную копию.
        </p>
        {limits && (
          <p style={MUTED}>
      ...
      <Panel title="Домены">
        ...
        {!writable && (
          <p style={MUTED}>
            Изменять логотипы доменов может тот, кому доверено оформление входа (право
            «branding.write»).
          </p>
        )}
      </Panel>
