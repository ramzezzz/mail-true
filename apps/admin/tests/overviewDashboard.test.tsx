...
const USERS = {
  ...
  total: 143,
  /*
   * Молчавших СЧИТАЕТ СЕРВЕР — по всем 143 ящикам, а не по отданной
   * странице. Их 118, и ни один в первые 25 строк не попал: страница
   * отсортирована по трафику по убыванию, молчащие стоят в хвосте.
   * Именно поэтому подсчёт по items давал ноль (здесь — единицу) и
   * выглядел при этом измеренным числом.
   */
  silent: 118,
  items: [
    {
...
describe('статистика по ящикам', () => {
  ...
  it('молчащие ящики считаются по всем, а не по показанной странице', async () => {
    await open();
    // 118 — число с сервера. Подсчёт по 25 показанным строкам дал бы 1
    // (и ноль на настоящем сервере, где молчащие в хвосте сортировки).
    expect(text()).toContain('Молчали за период: 118');
    expect(text()).not.toContain('Молчали за период: 1.');
  });
});
