...
const OWNER: Permission[] = [
  ...
  'backup.export',
  'backup.restore',
  // Настройки сервера и смена основного домена — обе целиком у владельца:
  // первое меняет поведение всего сервера, второе меняет адрес каждого
  // человека в организации и после переноса писем не отменяется.
  'serversettings.read',
...
describe('меню', () => {
  ...
  it('заглушек в меню не осталось', () => {
     ...
     * копию настроек, «Антиспам» ходит в контроллер rspamd (статистика,
  ...
  it('«Антиспам» и «Наблюдение» есть в меню и не помечены «скоро»', () => {
    const spam = NAV_ITEMS.find((i) => i.to === '/spam');
    const monitoring = NAV_ITEMS.find((i) => i.to === '/monitoring');
    expect(spam, 'в меню нет раздела «Антиспам»').toBeDefined();
    ...
    const titles = visibleNav(READONLY).map((i) => i.title);
    // Именно «Антиспам»: раздел управляет фильтром, а прежнее «Спам»
    // читалось как папка со спамом.
