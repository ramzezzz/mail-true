...
describe('нарезка длинного TXT не рвёт экранирование', () => {
  it('кавычка на границе куска не разрывается пополам', () => {
    /*
     * Экранирование превращает один знак в два, и нарезка по УЖЕ
     * экранированной строке могла разложить эту пару по разным кускам:
     * первый заканчивался обратной косой, второй начинался кавычкой — и
     * запись становилась синтаксически битой. Собираем значение так,
     * чтобы кавычка встала ровно на границу.
     */
    const value = 'a'.repeat(254) + '"' + 'b'.repeat(10);
    const text = zoneTxtValue(value);
    // Кусок не имеет права закончиться одиночной обратной косой: это и
    // означало бы разорванную пару.
    const parts = text.split('" "');
    expect(parts.length).toBe(2);
    for (const part of parts) {
      const trailing = /\\*$/u.exec(part.replace(/"$/u, ''))?.[0].length ?? 0;
      expect(trailing % 2).toBe(0);
    }
    // И собранное обратно значение совпадает с исходным.
    const joined = parts
      .map((part, index) => (index === 0 ? part.slice(1) : part.slice(0, -1)))
      .join('')
      .replace(/\\(.)/gu, '$1');
