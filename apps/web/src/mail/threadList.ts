...
/**
 * Выполнить действие над каждой порцией и узнать про КАЖДЫЙ отказ.
 *
 * ------------------------------------------------------------------
 * ЧТО БЫЛО
 * ------------------------------------------------------------------
 ...
 * запросе. Каждый следующий `mutate` отвязывает предыдущий запрос от
 * наблюдателя и затирает эти колбэки — значит `onError` позовут только у
 * ПОСЛЕДНЕЙ порции, а у всех прежних не позовут никогда.
 *
 * Цена этого в списке писем прямая. Выделено семьсот писем (в папке с
 * группировкой это одно нажатие «Выделить загруженные»), нажато
 * «Удалить», сервер отказал на первой порции. Человек видит «Не удалось
 * переместить письма» — и пятьсот строк остаются погашенными: их нельзя
 * ни выделить, ни открыть, ни повторить действие, и пометка «уезжает» с
 * них не снимется никогда, потому что письма из списка никуда не делись.
 * Лечится только уходом в другую папку. Ровно тот дефект, который в
 * FolderPage.tsx описан как вылеченный, — он вернулся через другую дверь.
 *
 * Здесь порции идут через `mutateAsync`: у него отказ приходит
 * исключением своей порции, и перепутать их нельзя по построению.
 * Ошибка каждой порции гасится (её уже показала сама мутация) — наружу
 * отдаётся список того, что не доехало.
 */
