 ...
 *
 * Строка та же, что понимает `parseAddresses` (lib/addresses.ts), и разбор
 * здесь свой намеренно: нужен ПЕРВЫЙ получатель и только он. Письмо,
 * начинающееся с «Здравствуйте, Пётр!», уходит и Петру, и Анне —
 * подставлять имя во множественной рассылке всё равно нельзя, а первый
 * получатель — тот, кому письмо адресовано на самом деле.
 */
export function firstRecipient(to: string): { name: string | null; address: string | null } {
  const part = to.split(/[,;]/u)[0]?.trim() ?? '';
  if (part === '') return { name: null, address: null };
  const match = /^(.*)<([^<>\s]+@[^<>\s]+)>$/u.exec(part);
  if (match?.[2]) {
    const name = (match[1] ?? '').trim().replace(/^"|"$/gu, '');
    return { name: name || null, address: match[2] };
  }
  // Голый адрес: имени мы не знаем и выдумывать его из левой части не
  // будем — см. `substitutionFor`.
  return { name: null, address: part.includes('@') ? part : null };
}
