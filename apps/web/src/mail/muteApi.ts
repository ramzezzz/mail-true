...
export const muteApi = {
  ...
  /**
   * Вернуть переписки ВЫДЕЛЕННЫХ ПИСЕМ.
   *
   * Ключ переписки в письме не лежит — его вычисляет сервер по
   * заголовкам. Раньше браузер вместо этого брал ключи всей подборки, и
   * «Вернуть переписку» разом расглушало всё, что человек прятал
   * неделями; а стоило заглушённых стать больше сотни, запрос упирался в
   * предел схемы и не работал вовсе.
   */
  async unmuteByMessages(ids: string[]): Promise<{ lifted: number }> {
    return apiFetch('/api/threads/mute', {
      method: 'DELETE',
      body: JSON.stringify({ ids }),
    });
