...
/**
 * Порядок писем НА ЭКРАНЕ — тот, по которому ходят стрелки.
 *
 * ------------------------------------------------------------------
 * ЧТО БЫЛО
 * ------------------------------------------------------------------
 * Клавиатура двигала курсор по сырому массиву `messages`, а рисуется
 * список из {@link flattenRows}, который строки ПЕРЕСТАВЛЯЕТ: письма,
 * вернувшиеся из «Отложенных», и письма без ответа поднимаются
 * отдельными группами наверх, а остальные раскладываются по периодам,
 * где период встаёт туда, где встретилось его первое письмо.
 *
 * Отсюда всё и ломалось. Во «Входящих» с одним вернувшимся письмом
 * человек жмёт стрелку вниз — курсор встаёт не на первую строку экрана,
 * а на вторую: поднятая группа пропускается целиком. Долистав до места,
 * где это письмо лежит по дате прихода, он получает рывок списка обратно
 * наверх без всякой причины. А Enter открывает не то письмо, на котором
 * он видит подсветку.
 *
 * Те же грабли были у стрелок «предыдущее/следующее» на странице письма:
 * соседи искались в сыром ответе сервера. Поэтому порядок считается один
 * раз здесь, и обе клавиатуры ходят по нему.
 */
export function orderedMessages(messages: readonly MessageSummary[], now?: Date): MessageSummary[] {
  const rows = flattenRows(messages, now);
  const out: MessageSummary[] = [];
  for (const row of rows) if (row.type === 'message') out.push(row.message);
  return out;
}
