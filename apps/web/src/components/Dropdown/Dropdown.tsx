...
export function Dropdown({
   ...
   *
   * Меряется ТОЛЬКО раскрытое меню. Здесь стояло «пока не закрыто», и
   * второй заход приходился на состояние 'closing' — на уже сдвинутое
   * меню. К этому времени оно стоит на экране целиком, новый сдвиг
   * выходит нулевым, и меню на время ухода отпрыгивает обратно за край:
   * человек видит, как оно уезжает не оттуда, где только что было.
   */
  useLayoutEffect(() => {
    if (state !== 'open') {
      // Закрытое меню начинает следующее открытие с чистого места
      if (state === 'closed') setShift(0);
      return;
    }
    ...
    <div ref={hostRef} className={cx(styles.host, className)}>
      ...
      {state !== 'closed' && (
        <div
          ...
          /*
           * Сдвиг двигает ТУ сторону, которой меню прибито к кнопке.
           *
           * Здесь стоял marginLeft — и для меню с align="right" он не
           * двигал ничего вовсе. Такое меню прибито правым краем
           * (right: 0), а левый край у него auto; в уравнении раскладки
           * этот auto и съедает внешний отступ слева, оставляя бокс на
           * месте. Замер в браузере показал ровно это: левый край меню
           * как был на 125 точек за экраном, так и остался — при
           * выставленном marginLeft в 52 точки. То есть срезанными
           * оставались все меню, которые правятся именно влево: панель
           * выделения писем, «Фильтр», меню ящика, меню темы, меню окна
           * написания.
           *
           * transform для этого не годится: ход появления (menuShow в
           * Dropdown.module.css) анимирует именно transform и на время
           * хода затёр бы сдвиг.
           */
          style={shift === 0 ? undefined : align === 'right' ? { right: -shift } : { left: shift }}
          className={cx(
            styles.menu,
