/**
 * Настройки папок (research/mailru/12-folders.png): таблица «Название папки —
 * Письма — действия», кнопка «Добавить папку».
 *
 * Набор действий у каждой строки СВОЙ и зависит от роли папки: Корзину
 * нельзя даже очистить, Спам нельзя переименовать, удалить можно только
 * пользовательскую папку. Логика прав — в lib/folderRights.ts, здесь она
 * только применяется.
 */
    ...
    >
      ...
      >
        ...
        {/* Вкладывать можно только в папки верхнего уровня: три уровня
            вложенности mail.ru не показывает, и мы не показываем. */}
        {folders
          .filter((f) => f.depth === 0)
          .map((f) => (
