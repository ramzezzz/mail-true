 ...
 * (перечень — docs/features-reference.md, «Общие настройки»).
 *
 * Форма работает по схеме «черновик — сохранение»: правки копятся в
 * состоянии и уходят на сервер одной кнопкой. Так же ведёт себя привычный почтовый интерфейс,
  ...
  return (
    <>
      ...
      <SettingsSection title="Список писем" description="Как выглядят строки в списке.">
        ...
        />
        {/*
          Группировка по переписке. Включена по умолчанию — так в привычных почтовых интерфейсах,
          и в этом заметная часть привычности списка. Описание называет
          и то, где она НЕ работает: молчаливое исключение читалось бы
          как поломка («включил, а в корзине не группирует»).
        */}
        <Switch
          label="Группировать письма в переписки"
          description={
            'Ответы на письмо занимают ту же строку, что и само письмо, а рядом с темой ' +
            'стоит число писем. Действия над строкой — удалить, в архив, прочитано, ' +
            'флажок — относятся ко всей переписке. В черновиках, корзине, спаме и ' +
            'отложенных письма всегда показываются по одному: туда заходят за одним ' +
            'конкретным письмом.'
          }
          checked={draft.groupByThread}
          onChange={(e) => patch({ groupByThread: e.target.checked })}
        />
      </SettingsSection>
