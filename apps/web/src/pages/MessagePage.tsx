...
export function MessagePage() {
  ...
  const listQuery: MessageListQuery = {
     ...
     * БЕЗ НАЧАЛ ПИСЕМ.
     *
     * Отсюда список нужен ровно для одного: узнать соседей открытого
     * письма, чтобы работали стрелки «предыдущее» и «следующее». Текста
     * в этих строках не видно нигде.
     *
     * А стоит он дорого: сниппет читается отдельной командой на КАЖДОЕ
     * письмо, последовательно, по единственному соединению ящика — до
     * сотни команд IMAP на страницу. Открытие письма делало это дважды
     * (второй раз без группировки, ради блока «Ещё писем в переписке»), и
     * ещё раз после автопометки «прочитано», которая сбрасывает весь кэш
     * списков. Итого до четырёхсот лишних команд на одно нажатие, и всё
     * это в очереди перед действиями человека.
     */
    snippets: false,
  };
