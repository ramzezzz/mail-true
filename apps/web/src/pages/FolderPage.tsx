      ...
      if (targetFolderId === folderId || targetFolderId === currentFolder?.role) {
        showNotice(
          targetFolderId === 'trash'
            ? 'Эти письма уже в «Корзине». Очистить её целиком можно в настройках — ' +
                'в списке папок или в разделе «Восстановление писем». Очищенные письма ' +
                'ещё несколько дней можно вернуть'
    ...
    <div className={styles.page}>
      <ListToolbar
        ...
        onFilterChange={(next) => {
          setFilter(next);
          /*
           * ВЫДЕЛЕНИЕ СБРАСЫВАЕТСЯ ВМЕСТЕ С ОТБОРОМ.
           *
           * Список после смены отбора ДРУГОЙ, а выделенные идентификаторы
           * оставались прежними — вместе с панелью действий, которая
           * бодро показывала «1» и «Удалить».
           *
           * Дороже всего это обходилось перепискам. Выделена строка из
           * шести писем, человек переключает «Непрочитанные», строки в
           * новом списке нет — и разворот строки в письма (expandThreadIds)
           * не находит её, отдавая идентификатор как есть. В «Корзину»
           * уезжает ОДНО последнее письмо разговора, пять остаются в
           * папке, ответ {moved: 1}, ни звука. Ровно тот дефект, против
           * которого написан весь threadList.ts, только через смену
           * отбора. Тем же путём ходили «Пометить непрочитанным»,
           * «Метки», «Отложить» и «Заглушить».
           */
          clearSelection();
          setFocusedId(null);
          clearVisitedMessage();
        ...
        */
        ...
        onLabelFilterChange={(key) => {
          // Та же причина, что и у смены отбора выше: список другой, а
          // действие ушло бы по идентификаторам из прежнего.
          setLabelFilter(key);
          clearSelection();
          setFocusedId(null);
          clearVisitedMessage();
