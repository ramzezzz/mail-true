 ...
 * Вход в почту.
 *
 * Оформление перенесено из отдельного прототипа (в репозитории его больше нет): тёмный фон с созвездием
...
export function LoginPage() {
  ...
  return (
    <div className={styles.page} data-paused={pausedAttr(visible)}>
      ...
      {/*
        Подвал: свой текст, если его задали в разделе «Оформление», иначе
        стандартные строки продукта. Переводы строк в своём тексте
        сохраняются — организации пишут туда телефон поддержки и порядок
        обращения, а это редко умещается в одну строку.
      */}
      <footer className={styles.footer}>
        {branding.loginFooter === null ? (
          ...
          branding.loginFooter
            .split('\n')
            .map((line, i) => <p key={`${String(i)}:${line}`}>{line}</p>)
        )}
      </footer>
    </div>
  );
