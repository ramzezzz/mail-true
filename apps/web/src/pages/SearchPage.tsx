            ...
            */}
            {facetsChosen ? (
              ...
              <>
                ...
                {/*
                  Число суммирует только те папки, которые ответили.
                  Упавшая даёт ноль — то есть «Найдено» заведомо занижено,
                  и молчать об этом нельзя: человек читает его как ответ на
                  «сколько всего» и перестаёт искать.
                */}
                {search.failedFolders > 0 && (
                  <span className={styles.totalNote}>
                    {' '}
                    · и это не всё: папок не опрошено — {search.failedFolders}
                  </span>
                )}
              </>
            )}
