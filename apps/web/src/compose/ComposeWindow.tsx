...
/**
 * Адрес картинки или ссылки в разметке.
 *
 * Без просмотра «от тега до атрибута»: такой разбор стоит квадрат от
 * длины письма и однажды уже останавливал сервер на минуты. Ищем сразу
 * атрибут, а слева требуем границу — иначе под образец попадали бы
 * `data-mt-src` и прочие свои пометки.
 *
 * Заглядывания назад (`(?<!…)`) здесь намеренно нет: Safari до 16.4 на
 * нём падает разбором ВСЕГО файла, а это вся почта целиком.
 */
const EMBEDDED_URL = /(^|[\s"'>])(?:src|href)\s*=\s*(["'])([^"']{0,4096})\2/gi;

/**
 * Содержимое письма для сравнения «пусто или нет».
 *
 * ------------------------------------------------------------------
 * ПОЧЕМУ НЕ ОДНОГО ВИДИМОГО ТЕКСТА
 * ------------------------------------------------------------------
 * `visibleText` снимает все теги — значит письмо, состоящее из ОДНОЙ
 * КАРТИНКИ, для него пусто. А на этом сравнении держатся три вещи:
 * идёт ли автосохранение, спрашивать ли при закрытии окна и мешать ли
 * уходу со страницы. Выходило так: человек вставил снимок экрана и
 * ничего не набрал — черновик не сохранялся НИ РАЗУ, а крестик и Esc
 * закрывали окно молча и сразу. Снимок исчезал без следа.
 *
 * Тем же путём терялись правки, не меняющие видимый текст: убрал из
 * черновика лишнее фото, закрыл окно — фото на месте.
 *
 * Поэтому к тексту добавляются адреса картинок и ссылок. Сравнивать
 * разметку целиком нельзя: браузер переписывает её по-своему (порядок
 * атрибутов, кавычки, пробелы), и окно считало бы изменившимся
 * нетронутое письмо — а это молчаливая перезапись черновика от одного
 * лишь открытия «посмотреть». Адреса же браузер сохраняет как есть.
 */
export function visibleContent(html: string): string {
  let out = visibleText(html);
...
export function ComposeWindow({
  ...
  /**
   * Запомнить черновик, который сервер спас после неудачной отправки.
   *
   * ------------------------------------------------------------------
   * ЗАЧЕМ
   * ------------------------------------------------------------------
   * Отказавшая отправка не выбрасывает письмо: сервер кладёт его в
   * «Черновики» новой версией и УДАЛЯЕТ прежнюю (saveDraftVersion).
   * Номер меняется, а окно об этом не узнавало ниоткуда — в ответе он
   * есть, но лежит в подробностях отказа, которых клиентская ошибка
   * раньше не несла.
   *
   * Отсюда две беды сразу. «Отменить» переносило в «Корзину» номер,
   * которого уже нет: человек соглашался с «написанное будет потеряно»,
   * а спасённый черновик оставался лежать — навсегда и без единого
   * признака. И следующее автосохранение считало себя первым, заводя
   * ВТОРОЙ черновик рядом со спасённым.
   */
  const noteRescuedDraft = (err: unknown): void => {
    if (!(err instanceof ApiError)) return;
    const details = err.details;
    if (typeof details !== 'object' || details === null) return;
    const uid = (details as { draftUid?: unknown }).draftUid;
  ...
  /** Уборка уже идёт: повторное «Отменить» не спрашивает второй раз. */
        ...
        onError: (err) => {
          ...
          if (options.silent === true) {
            ...
            if (failuresRef.current >= AUTOSAVE_MAX_FAILURES) {
              ...
              setError(text);
              /*
               * И общим извещением тоже: у СВЁРНУТОГО окна полосы с
               * ошибкой нет вовсе — свёрнутая плашка рисуется отдельной
               * веткой, где стоят только заголовок и крестик. Свернул
               * окно, потерялась сеть, за девять секунд три отказа,
               * автосохранение выключилось навсегда — и на экране ровно
               * та же плашка. Через час человек разворачивает окно и
               * узнаёт, что этот час не сохранялся. Соседний путь
               * (сохранение по кнопке) дублирует ошибку сюда же и ровно
               * по этой причине.
               */
              showNotice(text);
            }
  ...
  const untouchedBody = (): string => {
    ...
    return (
      visibleContent(signature ? signatureHtml(signature.text) : '') +
      visibleContent(win.init.bodyHtml ?? '')
    );
  };
  ...
  const hasContent = (): boolean =>
     ...
     */
    // Не visibleText: письмо из одной картинки — тоже письмо (см. visibleContent)
    visibleContent(currentBodyHtml()) !== untouchedBody();
  ...
  const discard = (): void => {
    /*
     * Второе нажатие, пока идёт уборка, не должно спрашивать заново.
     *
     * `cleanup` ждёт летящую запись — это сотни миллисекунд, всё это
     * время окно живо и кнопки нажимаются. Человек, не увидевший
     * отклика, жмёт «Отменить» второй раз и получает второй вопрос о
     * потере письма, а то и успевает нажать «Отправить» после того, как
     * уже согласился всё выбросить.
     */
    ...
    if (hasContent() || savedHere) {
      const question =
        continuingDraft && reallySaved
          ...
          : continuingDraft
            ? /*
               * Про «прежнюю версию» говорить нельзя: её уже нет.
               *
               * Автосохранение в этом окне шлёт номер открытого черновика,
               * сервер кладёт новую версию и УДАЛЯЕТ прежнюю. То есть к
               * моменту вопроса в «Черновиках» лежит не исходник, а почти
               * весь новый текст. Прежняя формулировка врала обоим: и
               * тому, кто отменял правки, чтобы вернуть исходник (его
               * уничтожили), и тому, кто вписал лишнее и «отменил» (оно
               * сохранено).
               */
              'Закрыть окно? Последние правки, которые не успели сохраниться, ' +
              'будут потеряны; в «Черновиках» останется то, что уже сохранено.'
            ...
            },
            {
              onSuccess: (result) => {
                if (result.cancelled) {
                   ...
                   * черновика сбрасываем — тот черновик сервер удалил,
                   * когда принял письмо, и ссылаться на него больше
                   * нельзя.
                   *
                   * ВМЕСТЕ С НИМ ЗАБЫВАЕМ И СЛЕПОК ПОСЛЕДНЕЙ ЗАПИСИ.
                   *
                   * Иначе выходило вот что. Человек пишет письмо,
                   * автосохранение кладёт черновик, он жмёт «Отправить» —
                   * сервер принимает письмо в очередь и УДАЛЯЕТ черновик.
                   * Человек тут же жмёт «Отменить отправку»: письмо
                   * возвращается в окно, но на сервере его теперь нет
                   * нигде. А слепок остался прежним, тело не менялось —
                   * значит автосохранение считает, что писать нечего, и
                   * предупреждение при уходе со страницы тоже молчит.
                   * F5 в эту секунду — и письма нет совсем.
                   *
                   * `null` означает «не сохраняли ни разу», то есть ровно
                   * то, что теперь и есть: следующая же пауза в наборе
                   * заведёт черновик заново.
                   */
                  savedShotRef.current = null;
