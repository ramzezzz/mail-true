/**
 * Права на действия с папкой и счётчик писем — раздел настроек «Папки»
 * (см. эталонные снимки интерфейса и таблицу в docs/features-reference.md).
 *
 * Права зависят от роли папки, а не от её имени: роль сервер определяет по
 * SPECIAL-USE, поэтому логика не ломается на серверах, где системные папки
 * названы иначе.
 *
 *   | Папка                   | Очистить | Переименовать | Удалить |
 *   | Входящие                | да       | нет           | нет     |
 *   | Вложенные автокатегории | да       | нет           | нет     |
 *   | Пользовательская        | да       | да            | ДА      |
 *   | Отправленные, Черновики | да       | нет           | нет     |
 *   | Спам                    | да       | нет           | нет     |
 *   | Корзина                 | ДА       | нет           | нет     |
 *
 * Переименование разрешено ровно там, где его разрешает сервер:
 * `PATCH /api/folders/:id` отвечает «Системную папку переименовать нельзя»
 * на любую папку с `system: true` (apps/api/src/settings/folders.ts).
 * Раньше интерфейс предлагал переименовать «Входящие», «Отправленные»,
 * «Черновики» и «Архив» — и каждая такая попытка заканчивалась ошибкой.
 */

import type { Folder } from '@mail-true/shared';

export interface FolderRights {
  /** «Очистить» — удалить все письма, саму папку оставить. */
  canClear: boolean;
  canRename: boolean;
  canDelete: boolean;
}

/** Что нужно знать о папке, чтобы посчитать права. */
export type FolderLike = Pick<Folder, 'role' | 'system'>;

export function folderRights(folder: FolderLike): FolderRights {
  switch (folder.role) {
    case 'trash':
      /*
       * Корзину ОЧИЩАТЬ МОЖНО — и это единственное место в продукте, где
       * такая кнопка вообще есть.
       *
       * Раньше здесь стояло canClear: false с объяснением «корзина и так
       * свалка удалённого». На деле это означало вот что: корзина росла
       * без предела и честно ела квоту ящика, а освободить её человек не
       * мог ничем — ни здесь, ни в самой папке, ни в «Восстановлении
       * писем» (та кнопка чистит уже ОЧИЩЕННОЕ, а не корзину).
       *
       * Заодно мёртвым лежал весь раздел «Восстановление писем»: список
       * возвращаемых писем наполняет только очистка корзины
       * (POST /api/folders/:id/clear — единственный вызов recovery.sweep
       * во всём коде), а вызвать её было неоткуда. Настройка «Сколько
       * хранить очищенное» при этом ни на что не влияла.
       *
       * Переименовать и удалить корзину по-прежнему нельзя: её путь —
       * часть соглашения с почтовыми клиентами (SPECIAL-USE \Trash) и
       * запасной приёмник правил фильтрации «удалить в корзину».
       */
      return { canClear: true, canRename: false, canDelete: false };
    case 'spam':
      // Спам чистится, но переименовать или удалить его нельзя:
      // на него завязано обучение антиспама.
      return { canClear: true, canRename: false, canDelete: false };
    case 'snoozed':
      /*
       * «Отложенные» нельзя ни очистить, ни переименовать, ни удалить —
       * и это строже, чем у остальных служебных папок.
       *
       * В ней лежат письма, которые сервер обязан вернуть, а найти их он
       * умеет по имени папки и по номеру письма в ней. Очистка папки
       * означала бы удаление писем, которых человек ЖДЁТ обратно и
       * которых больше нигде нет; переименование и удаление — то же самое
       * плюс потерянные сроки. Убрать письмо отсюда можно ровно одним
       * способом — «Вернуть сейчас», и он ничего не теряет.
       */
      return { canClear: false, canRename: false, canDelete: false };
    case 'muted':
      /*
       * «Заглушённые» очистить можно, а переименовать и удалить — нет.
       *
       * Разница с «Отложенными» ровно в том, чего человек ждёт от папки.
       * Из «Отложенных» письмо обязано ВЕРНУТЬСЯ, поэтому очистка означала
       * бы потерю ожидаемого письма. Здесь наоборот: сюда складывается то,
       * что человек читать не собирался, и «очистить» — законное желание.
       *
       * А вот имя папки записано в правило доставки Sieve
       * (apps/api/src/settings/sieve-muted.ts): переименуй её — и Dovecot
       * заведёт папку со старым именем заново, а человек будет искать
       * заглушённое там, где его нет.
       */
      return { canClear: true, canRename: false, canDelete: false };
    case 'inbox':
    case 'sent':
    case 'drafts':
    case 'archive':
      // Имена системных папок — часть соглашения с почтовыми клиентами
      // (SPECIAL-USE), сервер их переименовывать не даёт.
      return { canClear: true, canRename: false, canDelete: false };
    case 'custom':
      // Автокатегории («Рассылки», «Чеки») приходят с role=custom, но помечены
      // системными: это представления внутри «Входящих». Их сервер тоже не
      // даёт ни переименовать, ни удалить — решает флаг `system`.
      return { canClear: true, canRename: !folder.system, canDelete: !folder.system };
  }
}

/** Точный подсчёт больших папок дорог — свыше этого показываем «999+». */
export const COUNT_CAP = 999;

export function formatCount(value: number): string {
  return value > COUNT_CAP ? `${COUNT_CAP}+` : String(value);
}

/**
 * Счётчик писем в таблице настроек: у папки с непрочитанными — «10/999+»
 * (непрочитанные через дробь от общего числа), у остальных — только общее.
 */
export function formatFolderCount(folder: Pick<Folder, 'unreadCount' | 'totalCount'>): string {
  const total = formatCount(folder.totalCount);
  if (folder.unreadCount <= 0) return total;
  return `${formatCount(folder.unreadCount)}/${total}`;
}
