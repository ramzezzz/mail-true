 ...
 * (эталонные снимки интерфейса и 07-filter-all-actions.png).
...
export interface FilterRule {
   ...
   *
   * Приходит с сервера и только у сломанных правил. Так бывает после
   * переименования папки НЕ через нашу почту — по IMAP это разрешено из
   * любой почтовой программы, и правило остаётся указывать на старый
   * путь. Молча перенести его нельзя (папку могли и удалить), поэтому
   * строка правила говорит об этом прямо.
   */
  missingFolder?: string;
  /** Как соединять условия: все сразу или достаточно одного. */
  matchMode?: 'all' | 'any';
}
 ...
 * привычный почтовый интерфейс (эталонные снимки интерфейса). Правило без действий, наоборот,
...
/**
 * Действия правила списком — как в правой колонке списка фильтров привычных почтовых интерфейсах:
