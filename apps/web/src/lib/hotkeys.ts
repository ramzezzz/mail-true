...
/**
 * Сколько модальных окон открыто прямо сейчас.
 *
 * ------------------------------------------------------------------
 * ЗАЧЕМ СЧЁТЧИК
 * ------------------------------------------------------------------
 * Клавиши страницы и каркаса слушают `document`, а модальное окно
 * перехватывает только Escape и Tab — всё остальное проходит сквозь него
 * к списку писем ПОЗАДИ затемнения.
 *
 * Пока фокус стоит на поле или кнопке окна, клавиши и так игнорируются
 * (см. ignoreHotkeysFor). Но стоит щёлкнуть по тексту внутри окна — по
 * картинке в предпросмотре вложения, по строке в «Исходном тексте
 * письма», — и фокус уходит на неинтерактивный узел. С этого момента
 * «E» отправляет в архив письмо, которого человек не видит, «#» — в
 * корзину, а «C» открывает окно написания ПОД затемнением, откуда его не
 * достать. Ни одно из этих действий человек не просил и увидеть их не
 * может.
 *
 * Счётчик, а не флаг: окно поверх окна (подтверждение над формой) —
 * обычное дело, и закрытие внутреннего не должно возвращать клавиши,
 * пока внешнее ещё висит.
 */
let openModals = 0;

/** Окно открылось. Парная функция обязательна — иначе клавиши пропадут. */
export function noteModalOpened(): void {
  openModals += 1;
}

/** Окно закрылось. */
export function noteModalClosed(): void {
  openModals = Math.max(0, openModals - 1);
}

/** Открыто ли хоть одно модальное окно. */
export function anyModalOpen(): boolean {
  return openModals > 0;
}

export function ignoreHotkeysFor(target: EventTarget | null): boolean {
  return anyModalOpen() || isEditableTarget(target) || isInteractiveTarget(target);
}
