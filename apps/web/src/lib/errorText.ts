...
export function errorText(error: unknown, fallback = FALLBACK): string {
  if (isApiError(error)) {
    /*
     * 401 — это «сессия закончилась» НЕ всегда.
     *
     * С кодом AUTH_FAILED сервер говорит другое: логин или пароль не принял
     * ЧУЖОЙ сервер — тот, к которому подключён второй ящик. Своя сессия при
     * этом цела, и http.ts по тому же признаку не уводит на экран входа.
     * А текст «Сессия закончилась — войдите заново» здесь врёт дважды: и
     * про сессию, и про то, что делать. Поймано на отправке с
     * подключённого чужого адреса: пароль подключения протух, сервер
     * ответил «не принят логин или пароль, подключите ящик заново» — и
     * этот ответ подменялся предложением войти заново туда, откуда никто
     * не выходил.
     */
    if (error.status === 401 && error.code === 'AUTH_FAILED' && looksHuman(error)) {
