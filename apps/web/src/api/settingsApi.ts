 ...
 *
 * ВНИМАНИЕ: ни одного из перечисленных ниже маршрутов в API сейчас нет —
 * реализована только `httpSettingsApi`, повторяющая ожидаемый контракт.
 * По умолчанию (как и остальной интерфейс, см. `api/index.ts`) работают
 * заглушки, а `VITE_API_MOCK=0` переключает на настоящий сервер.
 *
 * Что нужно поднять на стороне API:
 *
