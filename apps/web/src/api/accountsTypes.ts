...
export interface ExternalSendRequest {
  /**
   * Черновик, из которого письмо отправляют, — чтобы сервер его убрал.
   *
   * Полей не было вовсе, и после отправки с чужого адреса черновик
   * оставался лежать в «Черновиках»: человек находил его через неделю,
   * не помнил, отправлял или нет, и отправлял ещё раз. У получателя дубль.
   * Свой путь отправки убирает черновик с самого начала.
   */
  draftUid?: number;
  draftKey?: string;
...
export interface ExternalSendResponse {
  /** false — письмо приняли не для всех получателей (см. `rejected`). */
  ok: boolean;
  /** Адрес, с которого письмо ушло. */
  from: string;
  /**
   * Кого чужой SMTP принял и кого отверг.
   *
   * Полей этих не было, потому что маршрут вовсе не смотрел на ответ
   * сервера: отказ ЧАСТИ адресов не отклоняет отправку целиком и лежит
   * внутри успешного ответа. Письмо на трёх адресатов, один из которых
   * не существует, закрывало окно с надписью «Письмо отправлено с адреса
   * …» — а третий не получал ничего, и узнать об этом было неоткуда.
   *
   * Необязательные: старый сервер их не присылает, и окно написания
   * обязано пережить ответ без них.
   */
  accepted?: string[];
