...
export type AiFeatureKey =
  ...
  | 'search'
  /**
   * Подсказка логотипов отправителей. Ключ был пропущен: сервер знает
   * восемь возможностей, а здесь их было семь. На отрисовку это не
   * влияло (список приходит с сервера), но любой перебор по этому типу
   * молча терял бы логотипы.
   */
  | 'logos'
  /**
...
export interface AiFeatureState {
   ...
   * Показываем это прямо: иначе человек включает и ждёт, что что-то
   * изменится, а не меняется ничего.
   */
  notInUi?: boolean;
