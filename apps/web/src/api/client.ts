  ...
  getMessages: (query) =>
    apiFetch(
      `/api/messages${buildQuery({
        ...
        label: query.label,
        /*
         * НАЧАЛА ПИСЕМ — ПО ПРОСЬБЕ, А НЕ ВСЕГДА.
         *
         * Сервер понимает `snippets=0` с самого начала, поле есть и в
         * контракте — а клиент не передавал его НИКОГДА, то есть быстрый
         * режим был мёртв со стороны браузера. Цена признака: сниппет
         * читается ОТДЕЛЬНОЙ командой на каждое письмо, последовательно,
         * по одному соединению — до сотни команд IMAP на страницу.
         *
         * Странице письма начала не нужны вовсе: она берёт из списка
         * только порядок, чтобы знать соседей для стрелок «предыдущее» и
         * «следующее».
         */
        snippets: query.snippets === false ? '0' : undefined,
      })}`,
    ),
