...
export interface GeneralSettings {
  ...
  /**
   * Идентификаторы подписей, которые были на экране при загрузке формы.
   *
   * Уходят только при сохранении: по ним сервер решает, что удалять, и
   * не сносит подписи, заведённые уже после загрузки — из админки или из
   * соседней вкладки.
   */
