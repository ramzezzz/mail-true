...
export interface SendResponse {
  ...
  undoUntil?: string;
  /**
   * Кого почтовый сервер не принял поимённо.
   *
   * Ответ приходит с кодом 200 и при частичном отказе: письмо УШЛО —
   * остальным получателям, — и объявлять отправку неудачной нельзя, иначе
   * человек отправит его второй раз и у них окажется дубль. Но и молчать
   * нельзя: молчание здесь читается как «дошло всем».
   *
   * Раньше этих полей в типе не было вовсе, и окно просто закрывалось.
   */
  rejected?: Array<{ address: string; message: string }>;
  /** Кого принял. */
  accepted?: string[];
  /** Легла ли копия в «Отправленные» — при переполненном ящике не ложится. */
  savedToSent?: boolean;
  /** Готовый текст про несохранённую копию; null — всё в порядке. */
  warning?: string | null;
}
 ...
 */
export interface UndoSendRequest {
  pendingId: string;
  /**
   * Окно написания держит это письмо у себя и вернёт его само.
   *
   * Шлётся ровно из одного места — полосы «Отменить отправку» в открытом
   * окне. Всё остальное (панель «Уйдут позже») этого поля не шлёт, и
   * сервер возвращает письмо в «Черновики»: окна там нет, а без возврата
   * «Отменить» означало бы «уничтожить письмо вместе с текстом».
   */
...
export interface SessionInfo {
  ...
  email: string;
  /**
   * Пределы, о которых окну написания надо знать ЗАРАНЕЕ.
   *
   * Сервер отдаёт их вместе с сессией (routes/auth.ts) — и отдавал давно,
   * но браузер их не читал. Без них файл заливался целиком и только потом
   * получал отказ: на тяжёлом файле человек ждал минутами, чтобы узнать,
   * что он не влезет. Своей копии числа здесь нет намеренно: предел
   * вложения — производная предела письма и меняется настройкой сервера,
   * а вторая копия разошлась бы с первой при первой же смене.
   */
  limits?: {
    /** Предел ОДНОГО вложения после кодирования. */
    attachmentBytes: number;
    /** Предел письма целиком. */
    messageBytes: number;
  };
}
...
export type WsEvent =
  ...
  /** IDLE-соединение потеряно — сервер поднимает наблюдение заново. */
  | { type: 'idle-lost' }
  /**
   * Наблюдение восстановлено после обрыва.
   *
   * Письма, пришедшие пока связи не было, отдельными событиями не
   * придут — по этому событию страница перечитывает списки и пропуск
   * закрывается. Уведомления о тех письмах не показываются: узнать о них
   * было неоткуда, и придумывать их задним числом мы не станем.
   */
  | { type: 'idle-restored' }
  /**
