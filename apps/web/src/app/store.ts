...
export interface ComposeDraft {
  ...
  /**
   * Вложения, которые ЕЩЁ ЕДУТ на сервер, но уже обещаны человеку.
   *
   * Заводится при пересылке: вложения исходного письма скачиваются и
   * заливаются обратно фоном, а окно открывается сразу — чтобы человек
   * набирал текст, не дожидаясь файлов. Пока счётчик больше нуля,
   * отправка недоступна.
   *
   * Без него письмо уходило БЕЗ файлов и молча: черновик собирается из
   * идентификаторов уже загруженных вложений, а «см. вложение» человек
   * пишет сразу. Загрузка потом дописывала файлы в уже закрытое окно, то
   * есть в никуда.
   */
...
export function emptyDraft(init: ComposeInit): ComposeDraft {
  ...
  return {
    ...
    cc,
    bcc,
    // Ничего не едет, пока пересылка не сказала обратного.
