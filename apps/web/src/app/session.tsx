...
export function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      /*
       * Чей это сеанс — запоминаем ЗДЕСЬ, а не только при входе через
       * форму.
       *
       * Раньше отметка ставилась исключительно в `login()`. А в почту
       * чаще заходят иначе: открыли вкладку, cookie ещё жива, форму
       * входа никто не видел — отметка осталась пустой. Дальше сессия
       * отваливалась (истёк срок, перезапустили сервер), за компьютер
       * садился другой человек, входил, и проверка «сменился ли адрес»
       * сравнивала его адрес с пустотой: закрытие чужих окон написания и
       * сброс чужого оформления не срабатывали. Он видел недописанное
       * письмо предыдущего — с адресатами и текстом, — и «Отправить»
       * ушло бы уже от его имени.
       */
      if (info.authenticated) lastAccount.current = info.email;
      /*
  ...
  const logout = useCallback(async () => {
    ...
    /*
     * НЕДОПИСАННОЕ ПИСЬМО НЕ ВЫБРАСЫВАЕТСЯ БЕЗ ВОПРОСА.
     *
     * Выход закрывает все окна написания (см. ниже — иначе следующий
     * вошедший на общем компьютере увидел бы чужое письмо). Но делал он
     * это молча: нажал «Выйти» — и начатое письмо исчезло. Автосохранение
     * спасает лишь то, что старше трёх секунд, да и то не всегда: у
     * письма без единой буквы текста черновика нет вовсе.
     *
     * Спрашиваем ДО выхода: после него окна уже не вернуть. Вопрос
     * задаётся по факту открытого окна, а не по содержимому, — знать о
     * набранном отсюда нечем, а лишний вопрос дешевле потерянного письма.
     */
    const open = useUiStore.getState().composeWindows.length;
    if (open > 0) {
      const what = open === 1 ? 'Начатое письмо' : `Начатые письма (${String(open)})`;
      if (!window.confirm(`${what} будет закрыто без сохранения. Всё равно выйти?`)) return;
    }
...
/**
 * Адрес открытого ящика — или пустая строка, если спросить не у кого.
 *
 * Отдельно от {@link useSession} и намеренно НЕ бросает. Нужен там, где
 * ответ «чей это ящик» украшает показ, а не управляет им: подпись «Кому:
 * вам» под адресом получателя. Такому месту незачем требовать провайдер
 * — не знаем адреса, значит просто напишем адреса получателей, как для
 * любого другого письма.
 *
 * Приведён к нижнему регистру: тот же ящик, записанный в письме с
 * заглавной буквы, — тот же человек.
 */
export function useAccountAddress(): string {
  const ctx = useContext(SessionContext);
