...
describe('folderRights', () => {
  ...
  /*
   * Корзину МОЖНО очистить — и это единственное место в продукте, где
   * такая кнопка есть.
   *
   * Пока стояло canClear: false, корзина росла без предела и честно ела
   * квоту ящика, а освободить её было нельзя ничем: ни здесь, ни в самой
   * папке, ни в «Восстановлении писем» (та кнопка стирает уже ОЧИЩЕННОЕ).
   * Заодно весь раздел «Восстановление писем» лежал мёртвым: его список
   * наполняет ровно очистка корзины (POST /api/folders/:id/clear —
   * единственный вызов recovery.sweep во всём коде), а позвать её было
   * неоткуда.
   */
  it('Корзину можно очистить, но не переименовать и не удалить', () => {
    expect(rights('trash')).toEqual({ canClear: true, canRename: false, canDelete: false });
  });
