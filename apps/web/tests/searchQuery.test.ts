...
describe('stemSearchQuery', () => {
  ...
  it('номер договора доезжает до сервера целым', () => {
    /*
     * Тема письма, вставленная в поиск. Знаки внутри номера («№», косая,
     * двоеточие) браузер трогать не должен ВООБЩЕ: что с ними делать,
     * знает только сервер — он один видит, каким полнотекстовым индексом
     * будет искать (см. ftsSafeText в packages/shared/src/search.ts).
     * Обрежь браузер здесь хоть что-то — и сервер получил бы не то, что
     * набрал человек.
     */
    expect(stemSearchQuery('Договор № 452/26: правки')).toBe('Договор № 452/26: правк');
  });
});
