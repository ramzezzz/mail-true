...
describe('правила для узкого экрана', () => {
  ...
  it('кнопка вызова папок скрыта по умолчанию и показывается на узком экране', () => {
    ...
    /*
     * Логотип уступает место поиску НЕ исчезновением, а сменой начертания.
     *
     * Раньше здесь стояло «на телефоне зона логотипа скрыта целиком» — так
     * и было сделано, потому что полное «MAIL.TRUE» съедало у поиска
     * половину строки. Заказчик увидел на снимке обратную сторону решения:
     * шапка осталась без марки и без пути на главную. Теперь на узком
     * экране показывается компактный знак, а полное начертание прячется —
     * поиску это не мешает (замерено: при окне 455 полю достаётся 227
     * точек и подсказка помещается целиком).
     */
    expect(narrowRules(css, 1024)).toMatch(/\.logoMark\s*\{[^}]*display:\s*block/);
    expect(narrowRules(css, 1024)).toMatch(/\.logoLight[\s\S]{0,400}display:\s*none/);
    expect(narrowRules(css, 480), 'знак должен остаться и на телефоне').not.toMatch(
