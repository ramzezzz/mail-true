...
export class ExternalImapPool {
  ...
  #makeClient(account: ExternalAccount, password: string): ImapFlow {
    return new ImapFlow({
       ...
       * шифрования»: дальше идёт STARTTLS. Но само по себе оно
       * НЕОБЯЗАТЕЛЬНО — сервер, который STARTTLS не объявляет, получит
       * пароль от чужого ящика и все письма открытым текстом. Различить
       * это человеку нечем: и STARTTLS, и «без шифрования» выглядят в
       * мастере одинаково — снятым флажком.
       *
       * ------------------------------------------------------------------
       * ЧТО БЫЛО
       * ------------------------------------------------------------------
       * Здесь стояло `requireTLS: true`. Такой опции у ImapFlow НЕТ вовсе
       * — это опция nodemailer (она верно применена в
       * admin/mail-roundtrip.ts). У ImapFlow требование STARTTLS
       * называется `doSTARTTLS`, и только при нём `_failSTARTTLS` рвёт
       * соединение вместо молчаливого продолжения открытым текстом.
       * Проверка типов лишнее свойство не поймала: оно приходило через
       * распаковку условного объекта.
       *
       * То есть защита не работала ни разу: снятый флажок «SSL/TLS» на
       * порту 143 означал, что каждые пятнадцать минут, вечно, пароль и
       * письма идут по сети открытым текстом, если сервер-источник
       * STARTTLS не объявил или его вырезал посредник. Мастер при этом
       * говорил «работает».
       *
       ...
       *
       * `doSTARTTLS` вместе с `secure: true` ImapFlow считает ошибкой
       * настройки, поэтому ставим его только когда начинаем без TLS.
       */
      ...(account.imap.secure || account.allowInsecureTls ? {} : { doSTARTTLS: true }),
...
export async function sendAsExternal(options: SendAsOptions): Promise<SendOutcome> {
  ...
  try {
    ...
    /*
     * НЕВЕРНЫЙ ПАРОЛЬ ЧУЖОГО ЯЩИКА — ЭТО НЕ «СЕРВЕР ОТКЛОНИЛ ПИСЬМО».
     *
     * Отказ при входе приходит с кодом 535 (реже 530 и 534), то есть из той
     * же пятисотой сотни, что и «нет такого получателя». Общего разбора для
     * него не было, и окно написания сообщало «Почтовый сервер отклонил
     * письмо (код 535)»: человек шёл проверять адреса и текст, хотя
     * поправить надо было ровно одно — пароль подключения (у почтовых
     * служб он к тому же протухает сам, когда включают двухшаговый вход).
     * Слова сервера при этом затирались нашими, а в них-то и написано,
     * что делать: «Invalid login: 535-5.7.8 Username and Password not
     * accepted» со ссылкой на страницу паролей приложений.
     *
     * Чтение того же ящика по IMAP этот отказ различает с самого начала
     * (см. verify и #open выше) — теперь так же и отправка.
     */
    if (failure.authFailed) {
      throw new AuthFailedError(
        `${smtp.host} не принял логин или пароль подключения «${account.address}» — ` +
          `подключите ящик заново. Ответ сервера: ${failure.serverText}`,
      );
    }
    if (failure.tooLarge) {
