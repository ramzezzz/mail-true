...
export function loadAccountsConfig(env: NodeJS.ProcessEnv = process.env): AccountsConfig {
  ...
  /*
   * Предел на один сбор не может быть длиннее срока «пометка брошена».
   *
   * ------------------------------------------------------------------
   * ЧТО БЫЛО
   * ------------------------------------------------------------------
   * Замок сам по себе правильный: `markCollectorStart` ставит пометку
   * одним атомарным UPDATE, и второй сбор её видит. Но пометка считается
   * брошенной через COLLECTOR_STALE_MINUTES — а предел на сбор
   ...
   * считается брошенным: рядом с ним запускается второй, и письма
   * копируются дважды.
   *
   * Настройка при этом выглядит совершенно разумной, а последствие —
   * дубли в чужом ящике — с ней не связывается никак.
   *
   * Сверять два значения умеет только это место: по отдельности каждое
   * в своих границах.
   */
  const timeoutMinutes = data.COLLECTOR_TIMEOUT_MS / 60_000;
  if (timeoutMinutes >= data.COLLECTOR_STALE_MINUTES) {
    throw new Error(
      `Некорректная конфигурация внешних ящиков: COLLECTOR_TIMEOUT_MS (${String(
        Math.round(timeoutMinutes),
      )} мин) не может быть больше или равен COLLECTOR_STALE_MINUTES (${String(
        data.COLLECTOR_STALE_MINUTES,
      )} мин). Иначе идущий сбор считается брошенным, рядом запускается второй, и письма ` +
        'копируются дважды. Поднимите COLLECTOR_STALE_MINUTES или уменьшите COLLECTOR_TIMEOUT_MS.',
    );
  }
