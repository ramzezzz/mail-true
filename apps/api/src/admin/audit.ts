/**
 * Журнал аудита: формирование и запись.
 *
 * Правила:
 *  - пишется КАЖДОЕ изменяющее действие (создание, изменение, удаление,
 *    вход в чужой ящик), даже если оно ничего фактически не изменило;
 *  - в old_value/new_value попадают только изменившиеся поля — читать
 *    журнал должно быть легко;
 *  - секреты (пароли, хэши, ключи) заменяются маркером '***' и никогда
 *    не попадают в базу в открытом виде;
 *  - записи не удаляются и не правятся — API на это ничего не предоставляет.
 */

/** Имена полей, значения которых нельзя писать в журнал. */
const SECRET_KEYS = [
  'password',
  'password_hash',
  'passwordhash',
  'passwd',
  'secret',
  'totp_secret',
  'token',
  'apikey',
  'api_key',
  'dkim_private_key',
];

/** Маркер, который подставляется вместо секрета. */
export const REDACTED = '***';

function isSecretKey(key: string): boolean {
  const normalized = key.toLowerCase().replace(/[^a-z_]/g, '');
  return SECRET_KEYS.some((s) => normalized === s || normalized.includes(s));
}

export type JsonValue = string | number | boolean | null | JsonValue[] | { [k: string]: JsonValue };

/** Рекурсивно заменяет значения секретных полей маркером '***'. */
export function redactSecrets(value: unknown): JsonValue {
  if (value === null || value === undefined) return null;
  if (Array.isArray(value)) return value.map((item) => redactSecrets(item));
  if (typeof value === 'object') {
    if (value instanceof Date) return value.toISOString();
    const out: Record<string, JsonValue> = {};
    for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
      out[key] = isSecretKey(key) ? REDACTED : redactSecrets(item);
    }
    return out;
  }
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
    return value;
  }
  // Объект в String() дал бы «[object Object]» — в журнале аудита это
  // запись «поле изменилось на [object Object]», по которой потом ничего
  // не восстановить. Разворачиваем в JSON.
  try {
    return JSON.stringify(value) ?? '';
  } catch {
    return '(значение не удалось прочитать)';
  }
}

function sameValue(a: unknown, b: unknown): boolean {
  if (a === b) return true;
  if (a instanceof Date && b instanceof Date) return a.getTime() === b.getTime();
  return JSON.stringify(redactSecrets(a)) === JSON.stringify(redactSecrets(b));
}

export interface ValueDiff {
  old: Record<string, JsonValue> | null;
  new: Record<string, JsonValue> | null;
}

/**
 * Оставляет только изменившиеся поля.
 * Поля, которых нет в `after`, не считаются изменёнными (частичное обновление).
 */
export function diffValues(
  before: Record<string, unknown> | null | undefined,
  after: Record<string, unknown> | null | undefined,
): ValueDiff {
  if (!before && !after) return { old: null, new: null };
  if (!before) return { old: null, new: redactSecrets(after) as Record<string, JsonValue> };
  if (!after) return { old: redactSecrets(before) as Record<string, JsonValue>, new: null };

  const oldOut: Record<string, JsonValue> = {};
  const newOut: Record<string, JsonValue> = {};
  for (const [key, next] of Object.entries(after)) {
    const prev = before[key];
    if (sameValue(prev, next)) continue;
    oldOut[key] = isSecretKey(key) ? REDACTED : redactSecrets(prev);
    newOut[key] = isSecretKey(key) ? REDACTED : redactSecrets(next);
  }
  const changed = Object.keys(newOut).length > 0;
  return changed ? { old: oldOut, new: newOut } : { old: null, new: null };
}

/** Кто выполняет действие. */
export interface AuditActor {
  id: number;
  login: string;
}

/** Откуда пришёл запрос. */
export interface AuditOrigin {
  ip: string | null;
  userAgent: string | null;
}

export type AuditTargetType =
  | 'user'
  | 'alias'
  | 'domain'
  | 'admin'
  | 'mailbox'
  | 'settings'
  /** Своё оформление входа: загрузка логотипа, возврат стандартного, подписи. */
  | 'branding'
  /**
   * Резервная копия настроек. В журнал попадают и выгрузка, и
   * восстановление: выгрузка отдаёт наружу файл с хэшами паролей, а
   * восстановление перезаписывает учётные записи — оба следа нужны.
   */
  | 'backup'
  /**
   * Перенос почты с чужого сервера. След обязателен: задание читает чужой
   * почтовый сервер под служебным доступом и пишет в наши ящики — то есть
   * трогает переписку людей, которые об этом не знают.
   */
  | 'migration'
  /**
   * Антиспам: списки отправителей и обучение фильтра. След обязателен и
   * ценен дольше остальных: запись в списке меняет приём почты для ВСЕГО
   * сервера, а обучение вообще нельзя отменить — только переучить. Через
   * полгода вопрос «почему письма из этого домена не приходят» решается
   * ровно одним запросом к этому журналу.
   */
  | 'antispam'
  /**
   * Настройки сервера. След обязателен и ценен дольше прочих: одна строка
   * здесь меняет поведение всего сервера — срок хранения истории, предел
   * размера письма, выключатель целого раздела. Через полгода вопрос
   * «почему письма старше недели пропали из истории доставки» решается
   * ровно одним запросом к этому журналу, потому что в нём записаны
   * и старое значение, и новое, и кто их поменял.
   */
  | 'serversettings'
  /**
   * Перезапуск службы стека. След обязателен и по-своему ценнее прочих:
   * это единственное действие в панели, которое ЗАМЕТНО СНАРУЖИ всем
   * сразу. Через полгода вопрос «почему у всех в 14:32 отвалилась почта
   * в Outlook» имеет ровно один способ получить ответ — эта запись, где
   * стоят и служба, и время, и человек.
   */
  | 'service'
  /**
   * Обновление продукта: свой код с пересборкой или свежие базовые
   * образы. След обязателен и переживает всё остальное: обновление
   * меняет сервер целиком, и вопрос «после чего это началось» решается
   * только этой записью — в ней стоят режим, время и человек. Отдельно
   * от 'service' потому, что перезапуск возвращает то же самое, а
   * обновление — другое.
   */
  | 'server';

export interface AuditInput {
  action: string;
  targetType: AuditTargetType;
  targetId?: number | null;
  targetLabel?: string | null;
  before?: Record<string, unknown> | null;
  after?: Record<string, unknown> | null;
}

/** Готовая к вставке строка admin_audit_log. */
export interface AuditRecord {
  adminId: number;
  adminLogin: string;
  action: string;
  targetType: AuditTargetType;
  targetId: number | null;
  targetLabel: string | null;
  ip: string | null;
  userAgent: string | null;
  oldValue: Record<string, JsonValue> | null;
  newValue: Record<string, JsonValue> | null;
}

/** Обрезает строку до предела столбца в базе. */
function clamp(value: string | null, max: number): string | null {
  if (value === null) return null;
  return value.length > max ? value.slice(0, max) : value;
}

/** Собирает запись журнала: диффует значения, вычищает секреты, режет длины. */
export function buildAuditRecord(
  actor: AuditActor,
  origin: AuditOrigin,
  input: AuditInput,
): AuditRecord {
  const diff = diffValues(input.before ?? null, input.after ?? null);
  return {
    adminId: actor.id,
    adminLogin: actor.login,
    action: input.action,
    targetType: input.targetType,
    targetId: input.targetId ?? null,
    targetLabel: clamp(input.targetLabel ?? null, 255),
    ip: clamp(origin.ip, 64),
    userAgent: clamp(origin.userAgent, 512),
    oldValue: diff.old,
    newValue: diff.new,
  };
}

/** Понятные названия действий для интерфейса журнала. */
export const ACTION_LABELS: Readonly<Record<string, string>> = {
  'admin.login': 'Вход в админку',
  'admin.login.failed': 'Неудачный вход в админку',
  'admin.logout': 'Выход из админки',
  /*
   * Имена — с «s», как их и пишет обработчик (routes/users.ts): ключи
   * 'admin.create' и 'admin.update' стояли здесь мёртвым грузом, а в
   * журнале в это время значилось машинное «admins.create». Пункт
   * «Создан администратор» человек не видел ни разу.
   */
  'admins.create': 'Создан администратор',
  'admins.update': 'Изменён администратор',
  'admins.password': 'Смена пароля администратора',
  'user.create': 'Создан ящик',
  'user.update': 'Изменён ящик',
  'user.password': 'Смена пароля ящика',
  'user.block': 'Ящик заблокирован',
  'user.unblock': 'Ящик разблокирован',
  'user.delete': 'Ящик удалён',
  'user.import': 'Массовый импорт ящиков',
  'user.bulk.quota': 'Массовая смена квоты',
  'user.bulk.active': 'Массовая блокировка/разблокировка',
  'alias.create': 'Создан алиас',
  'alias.update': 'Изменён алиас',
  'alias.delete': 'Удалён алиас',
  'domain.create': 'Добавлен домен',
  'domain.update': 'Изменён домен',
  'domain.delete': 'Удалён домен',
  'domain.dnscheck': 'Проверка DNS домена',
  'mailbox.impersonate': 'Вход администратора в ящик',
  'mailbox.impersonate.end': 'Выход из ящика пользователя',
  // Настройки чужого ящика из админки. Просмотр записывается наравне с
  // изменением: заглянуть в чужие фильтры и подписи — уже действие над
  // чужим ящиком, и владелец вправе узнать, что это делали.
  'usersettings.view': 'Просмотр настроек чужого ящика',
  'usersettings.general': 'Изменены настройки чужого ящика',
  'usersettings.filter.create': 'Создан фильтр в чужом ящике',
  'usersettings.filter.update': 'Изменён фильтр в чужом ящике',
  'usersettings.filter.delete': 'Удалён фильтр в чужом ящике',
  'usersettings.filter.order': 'Изменён порядок фильтров в чужом ящике',
  'usersettings.signature.bulk': 'Подпись по шаблону установлена в ящик',
  'usersettings.signature.bulk.run': 'Групповая установка подписей по шаблону',
  // Перенос почты с чужого сервера. Проверка связи тоже записывается:
  // это обращение к чужому серверу с чужими учётными данными, и след
  // от него нужен ровно так же, как от самого переноса.
  'migration.check': 'Проверка связи с исходным сервером',
  'migration.start': 'Запущен перенос почты',
  'migration.stop': 'Перенос почты остановлен',
  /*
   * Очередь. Действия писались в журнал и раньше, но названий у них не
   * было — в разделе «Журнал аудита» вместо строки по-русски стояло
   * служебное «queue.flush», и человек читал не то, что произошло, а
   * идентификатор из кода.
   *
   * Просмотр письма записывается наравне с ними, по той же причине, что
   * и usersettings.view: в письме очереди лежит переписка живых людей —
   * конверт, заголовки и начало тела. Заглянуть туда — это действие над
   * чужой перепиской, а не «просмотр сводки», и след от него нужен.
   */
  'queue.view': 'Просмотр письма в очереди',
  'queue.flush': 'Попытка доставить письмо из очереди',
  'queue.delete': 'Письмо удалено из очереди',
  /*
   * Резервная копия настроек. Разбор принесённого файла (preview) ничего
   * не меняет, но записывается наравне с выгрузкой и восстановлением:
   * его ответ — список адресов ВСЕХ ящиков сервера и логинов всех
   * администраторов, то есть адресная книга организации целиком. Это
   * такой же доступ к чужим данным, как вход в чужой ящик, и след от
   * него нужен ровно так же.
   */
  'backup.export': 'Выгружена копия настроек',
  'backup.preview': 'Разбор копии настроек (показан состав сервера)',
  'backup.restore': 'Настройки восстановлены из копии',
  /*
   * Антиспам. Списки правятся редко, а действуют на всю почту сервера:
   * через полгода «почему письма от этого партнёра не доходят» ищется
   * именно здесь.
   */
  'antispam.list.add': 'Добавлена запись в список антиспама',
  'antispam.list.remove': 'Удалена запись из списка антиспама',
  'antispam.learn.spam': 'Фильтр обучен: письмо — спам',
  'antispam.learn.ham': 'Фильтр обучен: письмо — не спам',
  /*
   * Настройки сервера. Возврат к умолчанию записывается отдельным
   * действием, а не изменением: «вернули как было» и «поставили то же
   * самое значение руками» — разные события. Первое означает, что
   * настройка снова следует за infra/.env и переедет вместе с ним;
   * второе — что она закреплена в базе и файл на неё больше не влияет.
   */
  'serversettings.update': 'Изменена настройка сервера',
  'serversettings.reset': 'Настройка сервера возвращена к умолчанию',
  /*
   * Перезапуск служб. Два действия, а не одно, и это не педантизм:
   * перезапуск поднимает тот же контейнер заново, а пересоздание строит
   * новый — с новым окружением, то есть с тем, что записано в infra/.env
   * на эту минуту. Разбирая потом «почему после этой кнопки поменялось
   * ещё и вон то», важно видеть, какое из двух действий было.
   *
   * Отдельная запись «служба не поднялась» нужна затем, что она означает
   * совсем другое событие: не «администратор перезапустил», а «сервер
   * после этого не работает». Искать её будут первой.
   */
  'service.restart': 'Служба перезапущена',
  'service.recreate': 'Контейнер службы пересоздан',
  'service.failed': 'Служба не поднялась после перезапуска',
  // Замена TLS-сертификата. В записи — отпечаток, имена и срок; самого
  // ключа нет ни здесь, ни где-либо ещё: журнал читают и выгружают люди,
  // которым доступ к приватному ключу не полагается.
  'tls.replace': 'Заменён TLS-сертификат сервера',
  /*
   * Смена основного домена. Записываются все три события, включая план:
   * составление плана выпускает ключ DKIM и показывает состав сервера
   * числами, а отмена объясняет, почему заведённый домен исчез. Без
   * записи о плане в журнале осталась бы дыра ровно там, где потом
   * спросят «откуда взялся этот домен и кто его завёл».
   */
  'domainchange.plan': 'Смена домена: составлен план',
  'domainchange.cancel': 'Смена домена отменена',
  'domainchange.apply': 'Смена домена запущена',
  /*
   * НАЗВАНИЯ ДЛЯ ВСЕГО ОСТАЛЬНОГО, ЧТО ПИШЕТСЯ В ЖУРНАЛ.
   *
   * Четырнадцать действий показывались машинным именем — «branding.texts»,
   * «tls.letsencrypt», «server.update». Читают журнал не те, кто писал код:
   * им это ровно та же строка, что и «ошибка 0x80070005». Проверка ниже
   * (audit-labels.test.ts) следит, чтобы новые действия не появлялись
   * снова без человеческого названия.
   */
  'branding.texts': 'Изменены надписи и оформление',
  'branding.logo.upload': 'Загружен логотип',
  'branding.logo.reset': 'Логотип возвращён к стандартному',
  'tls.letsencrypt': 'Выпущен сертификат Let’s Encrypt',
  'server.update': 'Запущено обновление сервера',
  'serversettings.secret.rotate': 'Сменён секрет сессий',
  'serversettings.secret.rotate.failed': 'Сменить секрет сессий не удалось',
  'ai.settings.update': 'Изменены настройки помощника',
  'ai.settings.test': 'Проверка связи с сервисом ИИ',
  'monitoring.roundtrip': 'Живая проверка прохождения письма',
  'monitoring.deletions.retry': 'Повтор незавершённого удаления ящика',
  /*
   * Просмотр последних писем антиспама. Записывается по той же причине,
   * что и письмо из очереди: на экране видны отправитель и ТЕМА письма,
   * то есть чужая переписка. До этой правки следа не оставалось вовсе.
   */
  'spam.history': 'Просмотр последних писем антиспама',
};

/** Название действия для интерфейса; неизвестное возвращается как есть. */
export function actionLabel(action: string): string {
  return ACTION_LABELS[action] ?? action;
}
