...
/**
 * Предпросмотр ничего не меняет — но след оставляет.
 *
 * Раньше эта проверка требовала обратного: «предпросмотр не меняет ничего
 * и в журнал не пишет». Первая половина верна и осталась, вторая была
 * ошибкой в самой постановке. Журнал аудита отвечает не только на вопрос
 * «кто что сломал», но и на вопрос «кто что видел», а ответ предпросмотра —
 * это адреса ВСЕХ ящиков сервера и логины всех администраторов, то есть
 * адресная книга организации целиком. Вход в один чужой ящик след
 * оставляет и требует причины; выгрузка всех адресов не оставляла ничего.
 */
test('предпросмотр копии показывает план, ничего не меняет и оставляет след', async () => {
  const h = await harness();
  const cookie = await h.cookie();
  ...
  // След есть, и это ровно один след, а не тишина и не поток записей.
  assert.equal(h.db.audits.length, before + 1, 'выгрузка адресной книги обязана оставлять след');
  const record = h.db.audits.at(-1) as Record<string, unknown>;
  assert.equal(record['action'], 'backup.preview');
  // В журнал уходят ЧИСЛА, а не сам список: класть туда те же адреса
  // значило бы размножить выгрузку, а не проследить её.
  const written = JSON.stringify(record);
  assert.doesNotMatch(written, /@/u, 'адреса ящиков в журнал попадать не должны');
  await h.app.close();
});
