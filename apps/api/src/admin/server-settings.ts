...
export interface ResolvedSetting {
  ...
  /**
   * Что лежит в окружении ПРЯМО СЕЙЧАС, у живого процесса.
   *
...
export class ServerSettings {
  ...
  #combine(spec: SettingSpec, stored: Stored | undefined): ResolvedSetting {
    const envRaw = fromEnv(spec, this.#env);
    if (stored !== undefined) {
      return {
        spec,
        raw: stored.value,
    ...
    if (envRaw !== undefined) {
      return { spec, raw: envRaw, source: 'env', envRaw, updatedBy: null, updatedAt: null };
    }
