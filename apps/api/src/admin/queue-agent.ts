...
export class QueueAgent {
   ...
   *
   * ------------------------------------------------------------------
   * ПОЧЕМУ ЗДЕСЬ ЧИТАЕТСЯ ВЫВОД, А НЕ ТОЛЬКО КОД ВОЗВРАТА
   * ------------------------------------------------------------------
   * `postsuper -d <id>` отвечает НУЛЁМ и тогда, когда удалять было
   * нечего: письмо ушло само, пока страница была открыта. Тело ответа
   * посредника при этом выбрасывалось целиком, и панель отвечала
   * «готово» на действие, которого не было: строка исчезала со страницы
   * (её и так уже не было в очереди), а в журнал уходила запись
   * «администратор удалил письмо» с одним лишь номером — потому что
   * сведений о письме в устаревшем снимке тоже не нашлось.
   *
   * Снимок очереди живёт три секунды, а вкладка открыта минутами:
   * попасть в это расхождение — не редкость, а обычный ход дел на живом
   * сервере. Postfix в таком случае просто молчит, а при настоящем
   * удалении печатает «Deleted: N message(s)» — по этому и отличаем.
   */
  async remove(queueId: string): Promise<number | null> {
    assertQueueId(queueId);
    this.cache = null;
    const body = await this.call(`/delete?id=${encodeURIComponent(queueId)}`, 'POST');
    return parseDeleted(typeof body.output === 'string' ? body.output : '');
  }
}

/**
 * Сколько писем удалил postsuper — по его же собственному отчёту.
 *
 * Возвращает `null`, когда понять нельзя: вывод есть, но незнакомый
 * (другая версия Postfix, чужая локаль). Врать в обе стороны нельзя,
 * поэтому «не знаю» — отдельный ответ, а не догадка.
 */
export function parseDeleted(output: string): number | null {
  const found = /Deleted:\s+(\d+)\s+message/i.exec(output);
  if (found) return Number(found[1]);
  // Пустой вывод у postsuper значит ровно одно: удалять было нечего.
  return output.trim() === '' ? 0 : null;
}
