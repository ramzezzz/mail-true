...
/**
 * Занятость тома в процентах — ОДНА формула на весь продукт.
 *
 * Считалась она в двух местах и по-разному, и это стоило прямого
 * противоречия на экране. «Наблюдение» брало usedBytes/totalBytes, то
 * есть (blocks − bfree)/blocks, а график истории на дашборде —
 * (total − свободно)/total, где «свободно» это bavail. Разница ровно на
 * резерв root, а он на ext4 по умолчанию 5 %: один и тот же диск
 * показывал 86 % (жёлтое «внимание») в одном разделе панели и 91 %
 * (красное «за порогом») в другом. Администратор, сверив два раздела,
 * переставал верить обоим.
 *
 * Выбрана формула ПО ДОСТУПНОМУ МЕСТУ: занято = (total − bavail) / total.
 * Причина та же, по которой свободное здесь и так берётся из bavail:
 * резерв root почтовым службам не отдадут никогда, и записывать его в
 * «свободно» значит обещать место, которого у Postfix и Dovecot нет.
 * Порог «диск кончается» обязан сработать ДО того, как приём почты
 * встанет, а не после.
 *
 * Занятое (usedBytes) при этом остаётся честным blocks − bfree: это
 * «сколько байт лежит на томе», и оно должно сходиться с df, а не с
 * процентом.
 */
