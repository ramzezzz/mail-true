...
test('голый механизм «a» в SPF — это разрешение, а не ошибка', () => {
  /*
   * «a» без аргумента разрешает отправку с адресов A-записи самого
   * домена. Запись «v=spf1 a -all» на домене, апекс которого указывает на
   * почтовый сервер, — рабочая и распространённая, а разбор её не знал:
   * строка краснела как «настроено с ошибкой».
   */
  const record = parseSpfRecord('v=spf1 a -all');
  assert.equal(spfAllowsHost(record, 'mail.example.ru'), 'yes');
  assert.equal(spfAllowsHost(parseSpfRecord('v=spf1 a:example.ru -all'), 'mail.example.ru'), 'yes');
});

test('чужой «a:» разрешением не считается', () => {
  // Обратный ход: проверка выше не должна зеленить что попало.
  assert.equal(spfAllowsHost(parseSpfRecord('v=spf1 a:chuzhoy.ru -all'), 'mail.example.ru'), 'no');
});
