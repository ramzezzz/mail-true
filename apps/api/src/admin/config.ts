...
export const adminEnvSchema = z.object({
  ...
  /*
   * Сроки хранения журналов панели.
   *
   * Раньше их не было вовсе: журнал аудита панели, справочник знакомых
   * адресов и журнал обращений к ИИ росли строкой на каждое действие и
   * не убирались никогда — а попадали в каждую резервную копию. Для
   * сравнения, у истории доставки и снимков показателей оба предела
   * (срок и потолок строк) есть с самого начала.
   *
   * Сроки разные и намеренно: журнал действий администраторов нужен при
   * разборе «кто это сделал» через месяцы, а расход ИИ интересен только
   * в пределах отчётного периода. Срок следов подбора пароля переехал
   * сюда из константы в janitor.ts — он был единственным зашитым в код.
   */
  ADMIN_AUDIT_RETENTION_DAYS: intVar(365, 7, 3650),
  AI_AUDIT_RETENTION_DAYS: intVar(180, 7, 3650),
  ADMIN_LOGIN_FAILURE_DAYS: intVar(30, 1, 3650),
  ...
  /**
   * Сколько ЗАДАНИЙ переноса вести одновременно.
   *
   * Не то же самое, что MIGRATION_CONCURRENCY: та ограничивает ящики
   * ВНУТРИ задания, а число самих заданий не ограничивал никто — работник
   * забирал все, что есть. Переезд по отделам (список на 50 ящиков, следом
   * второй, третий) означал столько живых заданий, сколько их успели
   * завести, и на каждое — свой пул соединений к базе. Postgres с
   * умолчанием max_connections = 100 отвечает «too many clients», а из той
   * же базы берут данные Postfix и Dovecot: сервер перестаёт принимать
   * почту и пускать людей в ящики. Очередь при этом никуда не девается —
   * задания просто разбираются по порядку, а не все разом.
   */
  MIGRATION_MAX_JOBS: intVar(2, 1, 16),
  /**
   * Сколько писем папки читать и переносить за один заход.
   *
   * Порция целиком лежит в памяти (метаописания писем: номер, размер,
   * флаги, дата, ключ дедупликации), поэтому расход памяти определяется
   * этим числом, а НЕ размером ящика. Раньше порций не было вовсе:
   * метаописания всех писем папки собирались разом, и INBOX на 300–500
   * тысяч писем клал процесс api по потолку кучи — вместе со всей
   * веб-почтой и админкой. Две тысячи — меньше мегабайта на порцию;
   * заметно меньше брать невыгодно (на каждую порцию приходится обращение
   * к серверу и запись курсора).
   */
  MIGRATION_CHUNK_SIZE: intVar(2000, 100, 100_000),
  /**
