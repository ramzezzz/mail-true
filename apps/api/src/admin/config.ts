...
export const adminEnvSchema = z.object({
  ...
  /**
   * Файлы миграций и каталог состояния установки — только на чтение.
   *
   * До них раздел «Наблюдение» не дотягивался и честно писал в списке
   * «чего не проверяет»: соответствие схемы файлам миграций и свежесть
   * резервной копии. Секретов там нет — SQL продукта и одна строка с
   * датой, — а вопросы это самые частые: докатана ли база до версии
   * продукта и когда в последний раз снимали копию.
   *
   * Пусто — каталог не смонтирован (так бывает на стенде разработки):
   * проверка тогда не выдумывает ответ, а честно говорит «не видно».
   */
  MIGRATIONS_DIR: z.string().default('/srv/migrations'),
  INSTALL_STATE_DIR: z.string().default('/srv/install-state'),
