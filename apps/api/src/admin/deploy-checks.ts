 ...
 *
 * ПРО МАЛЕНЬКОЕ ЧИСЛО ФАЙЛОВ. Раньше их было 36, теперь два: журнал
 * (0000) и вся схема разом (0001_baseline.sql) — 36 шагов разработки
 * свёрнуты в одну базовую схему. Смысл проверки от этого не изменился:
 * она по-прежнему ловит «код обновили, install.sh не запускали», просто
 * теперь непримененной чаще всего оказывается не двадцатая миграция,
 * а сама базовая схема. Новые изменения схемы будут приходить обычными
 * файлами 0003, 0004 и так далее, и считаться будут так же.
 *
 * Старые файлы лежат в подкаталоге legacy/ и в сверке не участвуют
 * намеренно: они нужны установщику лишь для доведения сервера, который
 * пропустил обновления, а на доведённом сервере их применённость ничего
 * не говорит о соответствии схемы продукту. Отбор по расширению .sql
 * подкаталоги и так не берёт (readdir не рекурсивен) — здесь это не
 * случайность, а условие.
 */
export function gradeMigrations(input: {
  ...
  if (missing.length === 0) {
    return {
      ...
      state: 'ok',
      /*
       * «Все N миграций применены» читалось нормально, пока файлов было
       * 36. После свёртки их два, и та же строка выходила бы «Все 2
       * миграций применены» — русский счёт требует другого окончания.
       * Проще не склонять вовсе, чем заводить ради одной фразы разбор
       * числительных.
       */
      detail: `Схема соответствует файлам миграций: ${String(files.length)} из ${String(files.length)}.`,
    };
  }
