...
export async function checkDomainDns(domain: string, options: DnsCheckOptions): Promise<DnsReport> {
  ...
  let wanted = checks.filter((c) => want(c.id));

  /*
   * ДОМЕН, КОТОРОГО НЕТ В ИНТЕРНЕТЕ.
   *
   * На стенде с доменом вида `home.local` — и вообще на любом домене,
   * зона которого ещё не делегирована, — красными становятся ВСЕ
   * проверки разом: A, MX, SPF, DKIM, DMARC, PTR. Это не поломка и не
   * недонастройка, а неприменимость: спрашивать записи не у кого,
   * потому что зоны не существует.
   *
   * Разница важна не для красоты экрана. Десяток красных строк, которые
   * невозможно починить, приучает не смотреть на этот раздел вообще — и
   * тогда настоящий отказ, когда он случится, тоже никто не заметит.
   *
   * Признаков два, и оба выводятся из УЖЕ полученных ответов — лишнего
   * запроса здесь нет намеренно: точечная проверка одной записи не
   * должна ходить в сеть за чем-то ещё.
   *
   *   1. Зона верхнего уровня зарезервирована (.local, .test, .internal
   *      и прочие): такой домен в интернете не существует по стандарту,
   *      и ждать ответа резольвера незачем.
   *   2. Резольверы отвечают ПО СУЩЕСТВУ (значит сеть в порядке), но по
   *      этому домену нет ничего вообще: ни адреса сервера, ни адреса
   *      самого домена, ни MX, ни SPF. Частично настроенная зона под это
   *      не попадает — там хоть что-нибудь да найдётся.
   *
   * Случай «резольвер не ответил вовсе» сюда не относится: там ответы
   * приходят как unreachable, и раздел уже говорит «не удалось спросить».
   * Путать «сеть не работает» с «домена нет» нельзя — чинить надо разное.
   */
  ...
  if (zoneMissing) {
    const why = reserved
      ? `зона «.${tld}» зарезервирована и в интернете не существует`
      : 'зона домена не делегирована: серверы имён для него не отвечают';
    wanted = wanted.map((check) =>
      check.status === 'ok'
        ? check
        : {
            ...check,
            // Не «не настроено», а «не проверено»: спрашивать не у кого.
            // «unknown» — единственный статус, который панель показывает
            // серым и не считает отказом.
            status: 'unknown',
            verdict: 'unreachable',
            diff: `Домен не опубликован в интернете — ${why}`,
            hint:
              'Пока домен не делегирован, записи проверить нечем. Это нормально для ' +
              'стенда и для домена, который ещё не завели у регистратора. На сервере, ' +
              'который должен принимать почту из интернета, это первое, что нужно ' +
              'исправить: без зоны в DNS до него не дойдёт ни одно письмо.',
          },
    );
  }
