...
test('каждый список объясняет, зачем он нужен, и показывает пример записи', () => {
  // Пустая таблица с надписью «Список пуст» не отвечает на единственный
  // вопрос, который перед ней возникает: что сюда вообще кладут. Из-за
  // этого разрешённые серверы (IP) путали с разрешёнными отправителями —
  // при том, что подделать можно ровно одно из двух.
  for (const list of SPAM_LISTS) {
    assert.ok(list.purpose.length > 60, `${list.id}: не объяснено, зачем список нужен`);
    assert.ok(list.example.length > 3, `${list.id}: нет примера записи`);
  }
  // Пример обязан быть таким, какой список примет: иначе подсказка учит
  // ошибке. Неправимые списки проверять нечем — там лежит либо домен
  // сервера, либо регулярное выражение.
  for (const list of SPAM_LISTS) {
    if (!list.editable) continue;
    assert.equal(checkEntry(list.value, list.example).ok, true, `${list.id}: пример не примут`);
  }
});
