...
export class AdminDb {
  ...
  /**
   * Задания импорта, которые уже никто не ведёт.
   *
   * ------------------------------------------------------------------
   * ЗАЧЕМ
   * ------------------------------------------------------------------
   * Импорт — единственное из трёх заданий раздела без работника: он идёт
   * прямо в процессе, принявшем запрос. Перезапуск контейнера посреди
   * работы оставлял строку в состоянии 'running' навсегда — состояния
   * 'queued' в таблице нет, брошенных никто не подбирал:
   *
   *   - страница импорта опрашивает такое задание раз в секунду
   *     бесконечно, показывая «идёт»;
   *   - смена основного домена запрещена, пока есть задание в 'running',
   *     с сообщением «Идёт массовое заведение ящиков», которое неправда.
   *     Строка уходила только по expires_at — то есть через НЕДЕЛЮ.
   *
   * Признак брошенности — молчание: updated_at обновляется при каждом
   * промежуточном сохранении (не реже чем раз в 25 строк, а строка — это
   * миллисекунды). Полчаса тишины у идущего импорта не бывает.
   */
  async failStaleImportJobs(staleMinutes: number): Promise<number> {
    const rows = await this.query<{ id: string }>(
      `UPDATE user_import_jobs
          SET state = 'failed',
              ...
              error = COALESCE(error,
                'Задание брошено: сервер перезапустился, пока шёл импорт. '
                || 'Уже созданные ящики остались, остальные строки можно завести повторно.')
        WHERE state = 'running'
  ...
  /**
   * Закрывает строки ящиков, оставшиеся в состоянии «переносится».
   *
   * ------------------------------------------------------------------
   * ЗАЧЕМ
   * ------------------------------------------------------------------
   * Задание закрывается двумя путями, которые строк ящиков не касались
   * вовсе: `expireStaleMigrationJobs` (работник замолчал) и исчерпание
   * попыток в работнике. После них ящик, который переносился в момент
   * обрыва, НАВСЕГДА оставался «переносится»:
   *
   *   - в панели у законченного задания висит вечное «идёт перенос»;
   *   - кнопка «Повторить неудавшиеся» его не берёт;
   *   - в подсчёт «перенесено / частично / не удалось» он тоже не
   *     попадает.
   *
   * То есть ящик, который не переехал, выпадал из повтора без единого
   * слова — при том что весь раздел построен вокруг обещания «повторите
   * неудавшиеся». Теперь такие строки честно помечаются `stopped`: это
   * ровно то состояние, которое повтор и подбирает.
   */
  async stopRunningMigrationItems(jobId: number, reason: string): Promise<number> {
    const rows = await this.query<{ id: string }>(
      `UPDATE mail_migration_items
          SET state = 'stopped',
              errors = COALESCE(NULLIF(errors, ''), $2::text),
              finished_at = COALESCE(finished_at, now()),
              updated_at = now()
        WHERE job_id = $1 AND state IN ('running', 'queued')
        RETURNING id::text`,
      [jobId, reason],
    );
    return rows.length;
  ...
  async expireStaleMigrationJobs(maxHours: number): Promise<number> {
    ...
    /*
     * Строки ящиков закрываем вслед за заданием.
     *
     * Без этого ящик, который переносился в момент обрыва, оставался
     * «переносится» навсегда: у закрытого задания вечное «идёт», повтор
     * его не берёт, в счётчики он не попадает. Ящик, который не переехал,
     * выпадал из повтора молча.
     */
    for (const row of rows) {
      await this.stopRunningMigrationItems(
        Number(row.id),
        'Задание брошено: работник не выходил на связь. Ящик можно повторить.',
      ).catch(() => 0);
    }
    return rows.length;
  }
