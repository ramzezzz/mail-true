...
export const SETTING_SECTIONS: ReadonlyArray<{ id: string; title: string; note?: string }> = [
  ...
  {
    id: 'fail2ban',
    title: 'Защита от подбора паролей и страна входа',
    note:
      'Служба читает журналы почты и веб-входа и закрывает адресу доступ ко ВСЕМ портам ' +
      'сразу, а не к одной форме. Пороги правят по ходу разбора — «нас заваливают, ' +
      'ужесточить» или «свой филиал забанился, ослабить», — поэтому они здесь, а не ' +
      'только в консоли. Значения читаются при создании контейнера, так что применяются ' +
      'пересозданием одной этой службы: на приём и доставку почты это не влияет. ' +
      'Рядом — страна входа: она ловит то, чего не ловит счёт неудачных попыток, — ' +
      'вход по УЖЕ подобранному паролю, который выглядит как обычный.',
  },
  {
    id: 'autoconfig',
...
const LIVE: readonly SettingSpec[] = [
  ...
  },
  {
    ...
    unit: 'days',
    description:
      'Сколько дней хранить журнал действий администраторов. Строка пишется на каждое ' +
      'действие в панели; срока у неё раньше не было вовсе, а в резервную копию журнал ' +
      'попадает целиком. Год — тот срок, на котором ещё отвечают на вопрос «кто это сделал».',
  },
  {
    key: 'AI_AUDIT_RETENTION_DAYS',
    section: 'panel',
    group: 'live',
    kind: 'int',
    def: '180',
    ...
    unit: 'days',
    description:
      'Сколько дней хранить журнал обращений к ИИ (кто спрашивал, сколько израсходовано). ' +
      'Нужен для учёта расхода за отчётный период; дальше это только вес в базе.',
  },
  {
    key: 'ADMIN_LOGIN_FAILURE_DAYS',
    section: 'panel',
    group: 'live',
    kind: 'int',
    def: '30',
    ...
    unit: 'days',
    description:
      'Сколько дней хранить следы неудачных входов в панель. Записи с действующей ' +
      'блокировкой не убираются, пока блокировка держит.',
  },
  {
...
const RESTART_SPECS: readonly SettingSpec[] = [
  ...
  },
  {
    ...
    description:
      ...
      'в чужой ящик, просроченные задания импорта), секунды. 0 — не запускать: вместе с ' +
      'ним перестают убираться следы неудачных входов и журналы панели по срокам хранения.',
  },
  {
    ...
    description:
      ...
      '(раздел «Почтовый поток» останется пустым). Ноль выключает и уборку уже собранной ' +
      'истории: сроки и потолок строк ниже перестают действовать, а накопленное остаётся ' +
      'в базе — сотрите его вручную или включите сбор обратно.',
  },
  {
  ...
  },
  {
    ...
    group: 'restart',
    kind: 'string',
    // Порта два: 5173 — почта, 5174 — панель. Здесь стоял только первый,
    // как и в схеме окружения, — а в compose перечислены оба. Кнопка
    // «вернуть к умолчанию» в панели откатила бы доступ панели к API.
    def: 'http://localhost:5173,http://127.0.0.1:5173,http://localhost:5174,http://127.0.0.1:5174',
    description:
  ...
  },
  {
    ...
    unit: 'perMinute',
    description:
      'Сколько раз в минуту к поиску логотипа привлекается помощник ИИ. Отдельный, ' +
      'меньший предел: список доменов приходит от почтового клиента и с перепиской не ' +
      'сверяется, а каждое обращение к модели списывается с общего дневного предела ' +
      'домена. 0 — не привлекать вовсе; логотипы тогда ищутся только по BIMI и значку сайта.',
  },
  {
...
const RECREATE: readonly SettingSpec[] = [
  ...
  /* --- страна входа --- */
  {
    key: 'GEOIP_LOGIN_POLICY',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'enum',
    def: 'off',
    options: ['off', 'log', 'allow'],
    applies: [{ target: 'api', action: 'recreate' }],
    description:
      'Что делать со страной входа: off — не определять; log — определять и писать в журнал ' +
      'ящика, пуская всех; allow — пускать только из стран списка. Проверяется ТОЛЬКО вход ' +
      'человека (почта и панель); входящую почту от чужих серверов по стране никто не ' +
      'фильтрует. Нужна база: install/fetch-geoip.sh. Без базы вход работает как раньше — ' +
      'проверка, которая при своей же поломке запирает всех, хуже отсутствующей.',
  },
  {
    key: 'GEOIP_ALLOWED_COUNTRIES',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'string',
    def: '',
    applies: [{ target: 'api', action: 'recreate' }],
    description:
      'Страны, из которых разрешён вход, — двухбуквенными кодами через запятую: RU, BY, KZ. ' +
      'Действует только при политике allow. Пустой список означает «не заполнили», а не ' +
      '«запретить всё»: запирать сервер из-за незаполненной настройки нельзя. Адреса из ' +
      'локальной сети не проверяются никогда, поэтому вход из офиса и резервный вход в ' +
      'панель по адресу сервера продолжают работать при любом списке.',
  },
  ...
  /* --- защита от подбора паролей --- */
  {
    ...
    kind: 'bool',
    def: 'true',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Банить адреса, с которых подбирают пароль. Включено по умолчанию: почтовый сервер ' +
      'начинают перебирать в первые же часы после того, как его порты увидят снаружи. ' +
      'Выключение НЕ гасит службу — она остаётся живой и зелёной, просто перестаёт ' +
      'разбирать журналы: «выключено» не должно выглядеть как «сломалось».',
  },
  {
    key: 'FAIL2BAN_IGNORE_IPS',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'string',
    def: '',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Адреса и сети, которые не банятся никогда, через пробел. Частные сети (10/8, ' +
      '172.16/12, 192.168/16), адрес самой машины и подсеть стека уже в списке — их ' +
      'дописывать не нужно. Сюда вписывают офис и филиалы. Ошибиться в этом поле ' +
      'опаснее всего: исправлять придётся с адреса, который к тому моменту, возможно, ' +
      'уже забанен.',
  },
  {
    key: 'FAIL2BAN_BANTIME',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'string',
    def: '1h',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'На сколько закрывается доступ после превышения порога. Формат fail2ban: 600, 10m, ' +
      '1h, 1d. Час выбран не из мягкости: перебор ботнетом всё равно идёт с новых ' +
      'адресов, а вот свой сотрудник со старым паролем в телефоне отсидит час и вернётся ' +
      'сам. Тех, кто вернулся подбирать снова, ловит правило о повторных.',
  },
  {
    key: 'FAIL2BAN_FINDTIME',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'string',
    def: '10m',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Окно, в котором считаются неудачи. Порог «15 неудач» означает «15 неудач за это ' +
      'окно», а не за всё время. Расширять окно — значит ловить медленный перебор ' +
      '(и заодно чаще ловить забывчивых), сужать — наоборот.',
  },
  {
    key: 'FAIL2BAN_DOVECOT_MAXRETRY',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'int',
    def: '15',
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Порог для получения почты (IMAP/POP3). Считается с недобором: одна строка журнала ' +
      'Dovecot — это соединение, внутри которого клиент мог перебрать до трёх паролей. ' +
      'Порог щедрый намеренно — главный источник ложных срабатываний здесь не ' +
      'злоумышленник, а забытый смартфон со старым паролем, который ходит за почтой ' +
      'каждую минуту.',
  },
  {
    key: 'FAIL2BAN_POSTFIX_MAXRETRY',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'int',
    def: '10',
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Порог для отправки с паролем (SMTP SASL). Строже, чем у получения, и это ' +
      'осмысленно: пароль на отправку спрашивают, когда человек отправляет письмо, а не ' +
      'в фоне каждые пять минут. Десять промахов подряд здесь — это уже не опечатка.',
  },
  {
    key: 'FAIL2BAN_API_MAXRETRY',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'int',
    def: '20',
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Порог для веб-почты и панели. Выше остальных, потому что эти формы и так прикрыты ' +
      'своими средствами: десять попыток в минуту с адреса и запирание пары ' +
      '«учётка+адрес». Здешний бан отличается тем, что закрывает адресу все порты, а не ' +
      'одну форму.',
  },
  {
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Наказывать возвращающихся: три бана за сутки — и адрес уезжает на неделю. Ложных ' +
      'срабатываний тут почти не бывает по устройству — сюда попадают только те, кого ' +
      'уже банили, причём трижды.',
  },
  {
    ...
    def: '1w',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description: 'На сколько закрывается доступ для возвращающихся. Формат тот же: 1w, 30d.',
  },
  {
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Сколько банов за сутки делают адрес возвращающимся. Двойка означает «второй бан — ' +
      'сразу на неделю», и это уже жёстко: два бана подряд бывают и у своего сотрудника, ' +
      'который переставил пароль не на всех устройствах.',
  },
  {
    key: 'FAIL2BAN_BANTIME_INCREMENT',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'bool',
    def: 'true',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Удваивать срок при каждом повторном бане одного и того же адреса. Тот, кто вернулся ' +
      'подбирать в третий раз, отсидит вчетверо дольше первого. Выключать стоит только ' +
      'если растущие сроки мешают разбираться.',
  },
  {
    key: 'FAIL2BAN_BANTIME_MAX',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'string',
    def: '7d',
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Потолок для растущего срока. Без него удвоение уводит срок в месяцы, а адреса ' +
      'переходят к другим людям: сегодняшний ботнет завтра окажется домашним интернетом ' +
      'вашего же сотрудника.',
  },
  {
    key: 'FAIL2BAN_LOGLEVEL',
    section: 'fail2ban',
    group: 'recreate',
    kind: 'enum',
    def: 'INFO',
    ...
    applies: [{ target: 'fail2ban', action: 'recreate' }],
    description:
      'Многословность журнала самой службы. INFO пишет каждый бан и разбан — это то, что ' +
      'нужно при разборе «почему у нас не открывается почта». DEBUG включают только на ' +
      'время: он пишет каждую прочитанную строку журналов почты.',
  },
