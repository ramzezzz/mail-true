...
const LIVE: readonly SettingSpec[] = [
  ...
  },
  {
    ...
    max: 100,
    unit: 'count',
    description: 'Неудачных попыток входа подряд до блокировки учётной записи администратора.',
  },
  {
...
const RESTART_SPECS: readonly SettingSpec[] = [
  ...
  },
  {
    ...
    description:
      ...
      'и обратный прокси, и панель; без кэша каждое обращение стучалось бы в Redis, ' +
      'Postgres и Dovecot.',
  },
  {
...
const LOCKED: readonly SettingSpec[] = [
  ...
  locked(
    ...
    'string',
    '0.0.0.0',
    'На каком адресе публиковать почтовые порты на боевой машине.',
    REASON_COMPOSE,
  ),
  ...
  locked(
    ...
    'Потолок кучи V8 у сервера приложения. Цель — VPS на 2 ГБ, поэтому задан явно.',
    'Читается процессом Node при ЗАПУСКЕ, до того как существует хоть какой-нибудь код, ' +
      'способный сходить в базу. ' +
      REASON_COMPOSE,
  ),
  locked(
    'POSTGRES_DB',
