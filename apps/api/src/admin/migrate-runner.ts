...
/**
 * Сколько раз подряд попытка выполнить задание может сорваться, прежде чем
 * оно будет закрыто с ошибкой.
 *
 * Пять — это примерно минута при проходе раз в десять секунд: перезапуск
 * контейнера базы или её пересоздание в такое окно укладываются, а
 * настоящая ошибка настройки — нет, и повторять её сотни раз в сутки
 * незачем.
 */
...
export class MigrationRunner {
  ...
  async runJob(job: MigrationJobRow): Promise<void> {
    ...
    try {
      ...
      /*
       * Считаем сорвавшиеся попытки, иначе безнадёжное задание крутится
       * вечно.
       *
       * Здесь было обещано, что оно «упрётся в срок молчания» и его
       * пометит брошенным сторож. Обещание невыполнимо по построению:
       * работник берёт задание каждые десять секунд и тем же запросом
       * обновляет отметку «жив», поэтому «молчит дольше 48 часов» не
       * наступает никогда. Отказ, который не пройдёт сам (неверная строка
       * подключения к хранилищу состояния, нет таблицы курсоров, права),
       * означал вечное «идёт»: каждые десять секунд новая попытка,
       * обращения к чужому серверу, пароли хранятся бессрочно. Выхода не
       * было ни одного — повтор для идущего задания запрещён, а
       * «Остановить» читается уже после подготовки, до которой дело не
       * доходит.
       *
      ...
      if (attempts >= MAX_JOB_ATTEMPTS) {
        await db
          .updateMigrationJob(id, {
            ...
            finished: true,
            error:
              `${message}. Подряд сорвалось попыток: ${String(attempts)} — задание закрыто. ` +
        ...
        await db
          .updateMigrationJob(id, {
            error:
              `${message}. Задание не закончено — работник возьмёт его снова, ` +
