...
/**
 * СТРОКИ, КОТОРЫЕ СИСТЕМА ПИШЕТ ПРО САМУ СЕБЯ.
 *
 * Проверка живости контейнера стучится в порты Dovecot и Postfix, а те
 * добросовестно записывают каждый стук. Настоящая доставка в этом потоке
 * теряется: на одно письмо приходятся десятки строк вида
 *
 ...
 *
 * Такие строки не удаляются и не перестают писаться — они честные и
 * иногда нужны (например, когда выясняют, отвечала ли служба вообще).
 * Они лишь помечаются, чтобы раздел журналов мог их скрыть, а человек —
 * вернуть одним переключателем.
 *
 * Признак — соединение БЕЗ попытки что-либо сделать: подключился,
 * поздоровался, отключился. Настоящий клиент так себя не ведёт: он либо
 * входит с паролем, либо отдаёт письмо. Поэтому здесь перечислены не
 * адреса служб (они меняются при пересоздании контейнеров), а именно
 * поведение.
 */
const SERVICE_NOISE: readonly RegExp[] = [
  // Dovecot: проверка портов IMAP/POP3 — соединение без единой попытки входа.
  /no auth attempts in \d+ secs/iu,
  // Dovecot LMTP: подключились и отключились на приветствии, письма не было.
  /Disconnect from .*state=GREETING/iu,
  /^lmtp\(\d+\): Connect from 127\.0\.0\.1$/iu,
  // Postfix: соединение, закрытое сразу после QUIT, без единой команды по делу.
  /disconnect from .* quit=1 commands=1$/iu,
  // Парная строка к предыдущей: сам факт подключения проверяющего.
  /^connect from mailtrue-[a-z-]+-\d+\./iu,
];

/**
 * Служебная ли это строка. Разбирается по тексту, а не по источнику:
 * тот же Dovecot пишет и настоящие входы в ящик, и отчёты проверки.
 */
export function isServiceNoise(text: string): boolean {
  return SERVICE_NOISE.some((re) => re.test(text.trim()));
}
