...
export async function checkAntispam(
  ...
  /*
   * Запоминаем ТОЛЬКО «подпись работает».
   *
   * Отрицательный вердикт — это состояние, которое чинят прямо сейчас:
   * администратор правит ключ DKIM и обновляет сводку. Отдавать ему пять
   * минут прежнее «исходящие уходят БЕЗ подписи» значит сказать, что
   * починка не помогла, — и он пойдёт ломать дальше то, что уже
   * исправлено. Возраст ответа на экране не показан, догадаться неоткуда.
   *
   * Цена честности здесь маленькая: пока подпись не работает, сводка
   * будет сканировать пробное письмо каждый опрос — но это состояние
   * редкое и недолгое, а счётчик антиспама всё равно уже сломан тем, что
   * подписи нет.
   */
  const remember = (dkim: ServiceCheck): { antispam: ServiceCheck; dkim: ServiceCheck } => {
    if (cacheMs > 0 && dkim.state === 'ok') {
      dkimProbeCache = { key: cacheKey, at: Date.now(), value: dkim };
    }
