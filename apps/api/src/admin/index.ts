...
export async function adminRoutes(app: FastifyInstance): Promise<void> {
  ...
  const ctx: AdminContext = {
    ...
    serviceAgent,
    // База стран одна на процесс: её читает и вход в почту, и вход в
    // панель. Второй экземпляр означал бы вторую копию десяти мегабайт
    // в памяти ради одной и той же таблицы диапазонов.
    ...(app.deps.geoip ? { geoip: app.deps.geoip } : {}),
    selfRestart,
