...
export async function adminQueueRoutes(app: FastifyInstance): Promise<void> {
   ...
   *
   * Право — mailbox.impersonate, то же, что у входа администратора в чужой
   * ящик. В журнале почты (audit.read) видны только адреса переписки, а
   * здесь — адреса, тема и начало текста, то есть само письмо: это ровно
   * то же самое, что открыть его в чужом ящике. Раньше стоял overview.read
   * — прочитать текст письма было ЛЕГЧЕ, чем строку журнала с одним
   * только адресом.
   *
  ...
  app.post(
    ...
    async (request) => {
      ...
      /*
       * «ПОНЯТЬ НЕЛЬЗЯ» — ТОЖЕ НЕ УСПЕХ.
       *
       * postsuper отчитывается строкой «Deleted: N message(s)». Если её в
       * выводе нет, а вывод при этом непустой (иная версия Postfix, чужая
       * локаль, предупреждение о правах на stderr — посредник отдаёт
       * stdout и stderr вместе), разобрать отчёт нечем. Прежний код
       * проверял только ноль, и такой случай проваливался в успех: письмо
       * оставалось в очереди и уходило получателю, а в журнале стояла
       * запись «администратор удалил письмо» с отправителем и адресатами.
       *
       * Записи не делаем и здесь: она означала бы уверенность, которой
       * нет. Человеку говорим правду — проверить придётся глазами.
       */
      if (deleted === null) {
        request.log.warn(
          { queueId: id },
          'Отчёт postsuper не распознан: удалено письмо или нет — неизвестно',
        );
        return {
          ok: false as const,
          reason: 'unknown' as const,
          message:
            'Почтовый сервер ответил непонятно: удалено письмо или нет — неизвестно. ' +
            'Обновите список и посмотрите, осталось ли оно в очереди.',
        };
      }
      await audit(ctx, request, {
        action: 'queue.delete',
