...
import { findLiveJob } from '../domain-change-jobs.js';
...
/**
 * Разбирает список разделов из поля формы.
 *
 * ПОЛЕ ОТСУТСТВУЕТ — все разделы (так шлёт старый клиент и так работает
 * восстановление «целиком»). ПОЛЕ ЕСТЬ, НО ПУСТОЕ — отказ.
 *
 * Разница важная: панель всегда шлёт поле, и пустое значение означает
 * «не выбрано ни одного раздела». Прежнее умолчание раскрывало это в
 * сторону наибольшего разрушения — восстанавливались ВСЕ семь разделов,
 * включая пароли ящиков и учётные записи администраторов. Держалось всё
 * на том, что кнопка в форме недоступна при пустом выборе.
 */
function parseSections(raw: unknown): BackupSection[] {
  if (raw === undefined || raw === null) return [...BACKUP_SECTIONS];
  if (raw === '') {
    throw new BadRequestError(
      'Не выбрано ни одного раздела для восстановления. Отметьте нужные — ' +
        'восстанавливать всё подряд по умолчанию опаснее, чем ничего.',
    );
  }
...
export async function adminBackupRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    async (request) => {
      ...
      /*
       * ВОССТАНОВЛЕНИЕ И СМЕНА ДОМЕНА ДРУГ ДРУГА НЕ ВИДЯТ — ЗАПРЕЩАЕМ.
       *
       * Восстановление, применённое во время смены домена, вписывает в
       * virtual_users адреса СТАРОГО домена уже после того, как каталоги
       * писем переименованы. Чьи строки победят — дело порядка коммитов,
       * а последствие одно: часть ящиков остаётся с адресом, под который
       * на диске каталога нет, Dovecot заводит пустые maildir, и люди
       * видят пустые ящики. Обе операции при этом отчитываются об успехе.
       *
       * Смена домена сама умеет перечислять препятствия (переносы,
       * выгрузки, импорты), но восстановление — не задание в базе, и
       * поймать его оттуда нечем. Значит проверяем здесь.
       */
      const liveChange = await findLiveJob(ctx.db).catch(() => null);
      if (liveChange) {
        throw new ConflictError(
          `Сейчас идёт смена основного домена на ${liveChange.newDomain}. Восстановление ` +
            'копии в этот момент вписало бы адреса прежнего домена поверх уже переехавших ' +
            'ящиков — часть людей осталась бы с пустыми ящиками. Дождитесь окончания.',
        );
      }
      ...
      await audit(ctx, request, {
        ...
        after: {
          ...
          mailboxesSkipped: outcome.skippedMailboxes.join(' '),
      ...
      return {
        ...
        aliasWarning:
          outcome.skippedAliases.length > 0
            ...
            : null,
        /**
         * Ящики, которые копия НЕ завела: их адрес занят живым
         * перенаправлением. Причина зеркальная предыдущей — Postfix
         * разбирает алиасы раньше ящиков, и такой ящик стоял бы пустым,
         * пока почта уходит в сторону. Самый вероятный путь сюда —
         * восстановление копии, снятой до смены домена.
         */
        mailboxWarning:
          outcome.skippedMailboxes.length > 0
            ? `Не восстановлено ящиков: ${String(outcome.skippedMailboxes.length)}. ` +
              `Их адрес занят перенаправлением (${outcome.skippedMailboxes.slice(0, 5).join('; ')}` +
              (outcome.skippedMailboxes.length > 5
                ? ` и ещё ${String(outcome.skippedMailboxes.length - 5)}`
                : '') +
              '). Заведи мы такой ящик, он был бы виден и пуст: письма продолжали бы уходить ' +
              'по перенаправлению. Уберите перенаправление, если ящик нужен.'
            : null,
        /**
