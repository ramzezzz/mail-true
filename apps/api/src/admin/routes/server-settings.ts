 ...
 *              значение с тем, С ЧЕМ ПРОЦЕСС СТАРТОВАЛ (envRaw):
 ...
 *
 *              Именно «с чем стартовал», а не «что в окружении сейчас».
 *              Сброс возвращает окружению файловое значение, и по
 *              текущему окружению признак получался бы ложно-спокойным:
 *              «вернулось к умолчанию, перезапуск не нужен» — о процессе,
 *              который до перезапуска живёт со сброшенным значением.
 *
...
function toDto(
  ...
  return {
    ...
    unit: spec.unit ?? null,
    /**
     * Пустое значение допустимо — например, «адрес наружу определяем
     * сами» или «свои резольверы не заданы».
     *
     * Признак объявлен в реестре давно, а в панель не передавался: там
     * любая пустая строка отвергалась как ошибка. Настройку, которую
     * администратор когда-то задал, нельзя было ОЧИСТИТЬ обратно — а
     * кнопки «вернуть к умолчанию» у значения из infra/.env нет.
     */
