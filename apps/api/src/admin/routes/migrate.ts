...
export async function adminMigrateRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const items = await ctx.db.listMigrationItems(id);
      /*
       * 'running' здесь — не «идёт прямо сейчас», а остаток.
       *
       * Выше уже проверено, что САМО задание закончилось. Значит строка,
       * оставшаяся в 'running', — это ящик, который переносился в момент
       * обрыва: перезапуск работника, исчерпанные попытки, брошенное
       * задание. Оба этих пути теперь закрывают такие строки сами, но
       * записи, сделанные до исправления, никуда не делись, а ящик,
       * который не переехал, обязан попадать в повтор. Пропустить его
       * молча — худшее из возможного: раздел построен вокруг обещания
       * «повторите неудавшиеся».
       */
