...
export const logoEnvSchema = z.object({
  ...
  /**
   * Сколько раз в минуту к подсказке логотипа привлекается помощник ИИ.
   *
   * Отдельный, гораздо меньший предел — потому что цена у этих обращений
   * другая. Список доменов приходит ОТ КЛИЕНТА и с почтой спрашивающего
   * никак не сверяется: для выдуманного домена ни BIMI, ни значок сайта
   * не отвечают, и дело доходит до третьего источника — модели. Каждое
   * такое обращение списывается с общего дневного предела ДОМЕНА, то
   * есть один человек мог за несколько минут оставить без помощника всю
   * контору.
   *
   * Кончился предел — логотип ищется без ИИ: это ровно то, что делает
   * продукт с выключенной подсказкой, и хуже от этого никому.
   */
  SENDER_LOGO_AI_PER_MINUTE: intVar(10, 0, 10_000),
