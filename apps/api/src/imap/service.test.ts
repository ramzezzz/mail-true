...
/*
 * Подсказка обещает «не позже этой даты», а IMAP BEFORE значит «строго
 * раньше этого дня». Разница в один день, и теряется весь названный:
 * `до:2026-08-01` не показывало ни одного письма от первого августа.
 * Человек в этот момент видит не «граница отбора такая», а «поиск врёт»:
 * письмо, которое он вчера читал, в отбор за месяц не попало.
 */
test('«до:» включает названный день, а не отрезает его', () => {
  const query = buildSearchQuery('all', 'до:2026-08-01');
  // Серверу называется СЛЕДУЮЩИЙ день: «раньше 2 августа» = «не позже 1-го».
  assert.equal(new Date(query.before ?? 0).toISOString(), '2026-08-02T00:00:00.000Z');
});

test('«после:» остаётся как есть — SINCE и так включает названный день', () => {
  const query = buildSearchQuery('all', 'после:2026-08-01');
  assert.equal(new Date(query.since ?? 0).toISOString(), '2026-08-01T00:00:00.000Z');
});
