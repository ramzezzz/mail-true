...
/* ------------------------------------------------------------------ */
/* Границы дат: «до:» включительно                                      */
/* ------------------------------------------------------------------ */

/*
 * Подсказка обещает «не позже этой даты», и это обещание раньше не
 * выполнялось дважды.
 *
 * imapflow при живом расширении WITHIN (Dovecot его объявляет) не шлёт
 * BEFORE вовсе — он считает «столько-то секунд назад» и шлёт OLDER.
 * Граница становится посекундной, и `до:2026-08-01` значит «старше
 * момента 1 августа 00:00»: весь первый день пропадает. А на сегодняшней
 * дате секунд выходит ноль, уходит `UID SEARCH OLDER 0`, и Dovecot
 * отвечает BAD — поиск отказывает целиком (проверено на стенде: 503).
 *
 * Обе беды лечит строка: её imapflow за дату не считает и отправляет
 * настоящее BEFORE, которое сравнивается по дню. Плюс сдвиг на день —
 * BEFORE у IMAP «строго раньше», а у нас «включительно».
 */
test('«до:» уходит строкой и включает названный день', () => {
  const query = buildSearchQuery('all', 'до:2026-08-01');
  // Строка, а не Date: иначе imapflow подменит BEFORE на OLDER в секундах.
  assert.equal(typeof query.before, 'string');
  // Следующий день: «раньше 2 августа» = «не позже 1-го».
  assert.equal(query.before, '2026-08-02T00:00:00.000Z');
});

test('«после:» тоже строкой, но без сдвига — SINCE день уже включает', () => {
  const query = buildSearchQuery('all', 'после:2026-08-01');
  assert.equal(typeof query.since, 'string');
  assert.equal(query.since, '2026-08-01T00:00:00.000Z');
});

test('обе границы вместе дают закрытый промежуток', () => {
  const query = buildSearchQuery('all', 'после:2026-08-01 до:2026-08-01');
  // Один-единственный день — самый частый случай «покажи за это число».
  assert.equal(query.since, '2026-08-01T00:00:00.000Z');
  assert.equal(query.before, '2026-08-02T00:00:00.000Z');
});

test('«старше:N» округляется до дня, а не до секунды', () => {
  // Отсчёт ведётся от «сейчас», поэтому проверяем форму, а не значение:
  // время суток должно быть срезано — иначе вернётся посекундный OLDER.
  const query = buildSearchQuery('all', 'старше:1г');
  assert.match(String(query.before), /T00:00:00\.000Z$/);
});
