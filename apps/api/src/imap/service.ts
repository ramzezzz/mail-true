  ...
  for (const item of listed) {
    ...
    if (!status && !item.flags.has('\\Noselect')) {
      ...
      try {
        ...
        /*
         * Папка может быть недоступна: её переименовали прямо сейчас,
         * повреждён индекс, отказано в доступе. Отказ здесь намеренно не
         * роняет весь список — одна больная папка не должна уносить всё
         * дерево, — но и в «ноль писем» он не превращается: статус
         * остаётся пустым, и mapFolders помечает такую папку
         * `countUnknown`. Ноль без этой пометки уже стоил переписки:
         * выгрузка ящика пропускала пустые папки и молча теряла те, чей
         * счётчик не прочитался.
         */
      }
    }
    raw.push({
...
export async function listMessages(
  ...
  try {
    ...
    /*
     * ОТБОР ПО МЕТКЕ, КОТОРЫЙ СЕРВЕР МОЛЧА ВЫБРОСИТ.
     *
     * imapflow добавляет условие KEYWORD в запрос ТОЛЬКО если ключевое
     * слово разрешено папкой: оно есть в PERMANENTFLAGS либо там стоит
     * `\*` («можно любые»). Иначе условие исчезает — без ошибки, без
     * предупреждения, — а в запросе остаётся ALL. То есть «письма с
     * меткой Оплатить» показывали ВСЮ папку целиком, под этим самым
     * заголовком и без единой пилюли метки в строках.
     *
     * Случай узкий, но настоящий: папка, открытая только на чтение, и
     * общая или чужая папка без `\*` в PERMANENTFLAGS. Ни проверки выше
     * (там сверяется только вид ключа), ни ниже (результат принимается
     * как есть) не было.
     *
     * Отвечаем отказом, а не полным списком: показать всю папку под
     * видом отбора — это соврать, а соврать здесь дороже, чем отказать.
     */
    if (query.keyword !== undefined) {
      // Папка может быть не открыта, а подделка в проверках про
      // PERMANENTFLAGS не знает вовсе: обе ситуации означают «сведений
      // нет», и запрещать по ним нельзя.
      const box = client.mailbox;
      const permanent =
        box && typeof box === 'object' && box.permanentFlags instanceof Set
          ? box.permanentFlags
          : undefined;
      const allowed =
        permanent === undefined ||
        permanent.has('\\*') ||
        permanent.has(query.keyword) ||
        // Набор может прийти без экранирования — сверяем и как есть.
        [...permanent].some((flag) => flag.toLowerCase() === query.keyword?.toLowerCase());
      if (!allowed) {
        throw new BadRequestError(
          `Папка не поддерживает метки, поэтому отобрать письма по метке в ней нельзя. ` +
            'Так бывает у папок, открытых только на чтение, и у общих папок.',
        );
      }
    }
