...
void test('отзыв согласия удаляет созданные резюме именно этого пользователя', async () => {
  ...
  try {
    ...
    /*
     * ОПИСЬ ПРИХОДИТ И С ОТВЕТОМ ИЗ ПАМЯТИ — раньше здесь стоял null.
     *
     * Опись описывает не сетевой вызов, а текст: что из письма попало в
     * работу и что из него ВЫРЕЗАНО. Без неё пропадала строка «переведено
     * без таких-то частей»: первое нажатие «Перевести» честно перечисляло
     * вырезанное, а назавтра то же письмо (ответ живёт 30 суток)
     * переводилось молча — при том что перевод заменяет письмо на экране.
     * Что наружу ничего не уходило, видно по `cached`, и интерфейс говорит
     * это отдельной строкой.
     */
    assert.ok(
      cached.disclosure,
      'опись нужна и для ответа из памяти: она про текст, а не про вызов',
    );
    assert.deepEqual(cached.disclosure, summary.disclosure, 'и она та же самая');
