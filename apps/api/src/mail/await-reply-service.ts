...
export class AwaitReplyService {
  ...
  async #scan(
    ...
    try {
      ...
      /*
       * ОТКАЗ ПОИСКА — НЕ «ОТВЕТА НЕТ».
       *
       * Здесь глотались все ошибки подряд, с оправданием про исчезнувшую
       * папку. Но захват замка обработан отдельно строкой выше, так что в
       * try остаются ровно поиск и чтение писем — а поиск умеет отказывать
       * по одной папке: повреждённый индекс Dovecot отвечает «Searching
       * mailbox INBOX failed», при этом «Отправленные» ищутся нормально.
       * Ради этого случая и написан searchUids: imapflow возвращает при
       * отказе мёртвый false, который иначе читается как «ничего не
       * найдено».
       *
       * Проглотив такой отказ, проверка отвечала «ответа нет» — и дальше
       * человеку клали во «Входящие» закреплённую копию его же письма с
       * пометкой «вам не ответили», хотя ответ лежал в тех же «Входящих»
       * третий день. Запись при этом закрывалась навсегда, повторной
       * проверки не бывает.
       *
       * Ошибаться эта проверка обязана в сторону «ответ был»: напомнить
       * зря — значит сказать неправду, а механизму, которому не верят,
       * лучше не существовать вовсе (см. шапку mail/await-reply.ts).
       * Поэтому отказ поиска пробрасываем: наверху он отметится попыткой,
       * и запись доживёт до следующего прохода.
       */
