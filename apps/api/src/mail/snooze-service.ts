...
export class SnoozeService {
  ...
  #syntheticRow(accountEmail: string, snoozePath: string, uid: number): SnoozedRow {
    return {
       ...
       *
       * Читаемая строка, а НЕ нулевой байт. Раньше здесь стоял настоящий
       * 0x00, и от него было три беды: файл считался двоичным (поиск по
       * коду его пропускал — на этом я потерял время, разыскивая функцию
       * внутри собственного исходника), такой символ незаметен глазом при
       * чтении, и он способен уехать в команду IMAP, где ему делать нечего.
       * Двоеточие в имени тоже не случайно: разделитель иерархии у Dovecot
       * — точка, поэтому такого пути в ящике не будет.
       */
      originPath: 'mail-true:нет-такой-папки',
      messageId: null,
      subject: '',
      fromAddress: '',
