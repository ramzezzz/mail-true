...
export function mapFolders(list: RawFolderInfo[]): Folder[] {
  ...
  for (const { info, role } of withRoles) {
    ...
    folders.push({
      ...
      /*
       * Счётчика нет — говорим об этом вслух, а не выдаём ноль за правду.
       *
       * STATUS по папке может не пройти (папку переименовали прямо сейчас,
       * повреждён индекс, отказано в доступе), и listFolders такой отказ
       * намеренно не роняет наверх: одна больная папка не должна уносить
       * всё дерево. Но `?? 0` делал «не прочитали» неотличимым от «пусто»,
       * а на этом счётчике принимаются решения — выгрузка ящика пропускала
       * такие папки как пустые и теряла переписку целой папкой, ничего не
       * сказав. Поле необязательное: у прочитанной папки его нет вовсе.
       */
      ...(info.status === undefined ? { countUnknown: true } : {}),
