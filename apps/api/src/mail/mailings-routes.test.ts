...
test('две отметки осмотра подряд не совпадают даже в одну миллисекунду', async () => {
  /*
   * Отметка — единственная защита от уборки по устаревшим числам: клиент
   * присылает ту, что видел, и разошедшиеся отметки означают отказ.
   *
   * Раньше отметкой было голое `new Date().toISOString()` — с точностью
   * до миллисекунды. Два осмотра, уложившиеся в одну и ту же
   * миллисекунду, получали ОДИНАКОВУЮ отметку, и старый снимок проходил
   * проверку как свежий: человек видел «убрать 40 писем», а уезжало
   * другое количество. На быстром ящике (а поддельный клиент в проверке
   * рядом — как раз такой) это случалось примерно через раз, отчего та
   * проверка и падала через раз.
   */
  const { app } = await buildHarness();
  const first = await mailings(app);
  const second = await app.inject({ method: 'GET', url: '/api/mailings?refresh=1' });
  const at = second.json<{ at: string }>().at;
  assert.notEqual(at, first.at, 'совпавшая отметка пропустит уборку по устаревшим числам');
  await app.close();
});
