...
const SYSTEM_KEYWORDS = new Set([
  ...
  '$Snoozed',
  // Ожидание ответа (mail/await-reply-service.ts): «ждём ответа» на
  // отправленном письме и «ответа нет» на поднятой копии. Обе пометки
  // ставит и снимает сервер — в ряду пользовательских меток им не место
  // по той же причине, что и `$Snoozed`.
  '$AwaitReply',
  '$NoReplyYet',
]);
...
export function buildSummary({
  ...
  return {
    ...
    /*
     * Ожидание ответа. Два состояния, а не одно, потому что показываются
     * они по-разному и означают разное: «ждём ответа» — спокойная пометка
     * на отправленном письме, «ответа нет» — то, ради чего возможность и
     * делалась. Решение опять же принимает сервер по ключевому слову:
     * пометка живёт в письме и одинакова во всех вкладках.
     */
    awaitReply: msg.flags?.has('$NoReplyYet')
      ? 'overdue'
      : msg.flags?.has('$AwaitReply')
        ? 'waiting'
        : undefined,
