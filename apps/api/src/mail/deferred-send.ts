...
export class DeferredSpool {
  ...
  async bumpAttempt(id: string): Promise<number> {
     ...
     *
     * Здесь единственным местом оставалась запись ПОВЕРХ, и она нарушала
     * инвариант, объявленный в шапке этого файла: наличие `.json` значит
     * «запись целая». Пропадание питания посреди перезаписи оставляло
     * обрезанный конверт: `get()` возвращал бы null, `all()` пропускал бы
     * такую запись — письмо не ушло бы никогда, извещения не было бы, а
     * `.eml` с телом остался бы лежать в очереди навсегда (уборщика у неё
     * нет). Тихая потеря письма ценой одной строки.
     *
    ...
    await rename(tmp, this.metaPath(id));
