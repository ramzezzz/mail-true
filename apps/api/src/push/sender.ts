...
export async function sendPush(options: SendPushOptions): Promise<PushSendResult> {
  ...
  try {
    ...
    /*
     * Тело чужого ответа НАРУЖУ НЕ ОТДАЁМ.
     *
     * Оно доезжало до панели уведомлений («проверочное уведомление»
     * показывает `error`), и вместе с подпиской на произвольный адрес это
     * складывалось в чтение ответов оттуда, куда снаружи не достучаться.
     * Адрес подписки теперь проверяется (endpoint-guard.ts), но пересказ
     * чужого ответа человеку не нужен и в исправном случае: службы
     * доставки отвечают своими кодами, а не объяснениями.
     *
     * В журнал сервера текст пишем — там он и нужен, когда служба
     * доставки начинает отвечать отказом всем подряд.
     */
    const text = await response.text().catch(() => '');
    if (text !== '') {
      options.logger?.warn(
        { status: response.status, body: text.slice(0, 300) },
        'Служба доставки уведомлений ответила отказом',
      );
    }
    return {
      ok: false,
      ...
      gone,
      error: `Служба доставки ответила ${String(response.status)}`,
    };
  } catch (err) {
    ...
    //
    // Наружу отдаём обобщённо: сообщение сетевой ошибки описывает
    // внутреннее устройство (имена узлов, коды резолвера) и человеку у
    // кнопки «проверить» ничего не объясняет. Подробности — в журнал.
    options.logger?.warn({ detail: message.slice(0, 300) }, 'Уведомление не удалось отправить');
    return {
      ok: false,
      status: 0,
      gone: false,
      error: 'Служба доставки уведомлений не ответила',
    };
  } finally {
    clearTimeout(timer);
  }
