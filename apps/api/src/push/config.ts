...
export const pushEnvSchema = z.object({
  ...
  /**
   * Сколько дней подписка может подряд отказывать, прежде чем её уберут.
   *
   * Две недели — заведомо больше отпуска и любого разумного простоя
   * службы доставки, и заведомо меньше, чем «навсегда», как было раньше.
   * Живой подписке уборка не грозит: успешный заход обнуляет отметку.
   */
  PUSH_DEAD_AFTER_DAYS: intVar(14, 1, 365),
});
