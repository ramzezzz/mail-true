...
export class PushDb {
  ...
  /**
   * Снимает ВСЕ подписки ящика.
   *
   * Нужно при закрытии доступа: смена пароля, блокировка,
   * восстановление копии. Пароль и признак «включён» не выгоняют никого
   * сами по себе, а подписка на уведомления живёт отдельно от сессии —
   * и продолжала работать. Уволенный сотрудник, у которого отобрали
   * доступ, получал на свой браузер тему и отправителя каждого нового
   * письма: подписка есть, сессия для этого не нужна.
   */
  async deleteSubscriptionsOf(accountEmail: string): Promise<number> {
    const rows = await this.#query<{ id: string }>(
      `DELETE FROM push_subscriptions WHERE lower(account_email) = lower($1) RETURNING id`,
      [accountEmail],
    );
    return rows.length;
  }
