...
export async function composeRoutes(app: FastifyInstance): Promise<void> {
  ...
  async function materializeDraft(
    ...
    /*
     * УЦЕЛЕВШИЕ КОПИИ НЕГОДНОГО НАБОРА НЕ УДАЛЯЮТСЯ.
     *
     * Здесь стояло удаление — и это был размен утечки на потерю. Набор
     * считается негодным, когда пропала ХОТЬ ОДНА его часть; остальные
     * при этом живы, и на них смотрит окно, которое человек открыл
     * раньше и не закрывал: в его теле стоят ссылки на эти самые номера.
     * Снеся их, мы превращали одну потерянную картинку в потерянные все.
     *
     * Утечка, ради которой это делалось, почти исчезла сама, когда ключ
     * памяти сменился с номера черновика на отпечаток частей: повторные
     * открытия теперь переиспользуют набор, а не плодят копии. Остаток —
     * один набор на отпечаток, и тот живёт до суток. Это работа уборщика,
     * а не наша.
     */
