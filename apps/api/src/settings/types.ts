 ...
 *
 * Набор полей взят один в один с интерфейса настроек привычный почтовый интерфейс
 * (docs/features-reference.md, разделы «Общие настройки» и «Правила
...
export interface MailSettings {
   ...
   *
   * По умолчанию ВКЛЮЧЕНО — так же, как в привычных почтовых интерфейсах, который мы повторяем.
...
/** Значения действий по умолчанию (совпадают с эталонной вёрсткой). */
...
export function defaultMailSettings(email: string): MailSettings {
  return {
    ...
    // Включена намеренно: в привычных почтовых интерфейсах группировка по переписке — поведение
