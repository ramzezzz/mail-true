...
test('«не успели за отведённое время» — это не ошибка, а «часть писем потеряли» — ошибка', () => {
  /*
   * Оба случая приходят одним внутренним состоянием 'partial', и
   * различает их только число непереехавших писем. Пока различия не
   * было, первая синхронизация большого ящика — она в предел не
   * укладывается никогда — красила подключение красным и звала чинить
   * исправное.
   */
  assert.equal(toWebStatus('partial', 0), 'ok');
  assert.equal(toWebStatus('partial', 3), 'error', 'потерянные письма — это ошибка, а не «ок»');
});
