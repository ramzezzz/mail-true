...
export interface WebCollector {
  ...
  error: string | null;
  /**
   * Что сказать про НЕЗАКОНЧЕННЫЙ сбор — обычным шрифтом, не красным.
   *
   * Первая синхронизация большого ящика в отведённое время не
   * укладывается никогда: сбор переносит сколько успел и продолжит с
   * курсора в следующий заход. Это не ошибка, но и не «обновлено» —
   * человеку нужно знать, что почта ещё едет, иначе он решит, что
   * приехало всё, и не дождётся остального.
   */
  note: string | null;
}
 ...
 *
 * Внутренних состояний пять, в контракте три, и 'partial' делится на два
 * разных случая — поэтому одного имени состояния мало, нужно число
 * непереехавших писем.
 *
 * ------------------------------------------------------------------
 * ЧТО БЫЛО
 * ------------------------------------------------------------------
 * 'partial' показывался как ошибка всегда. А сборщик ставит 'partial' и
 * тогда, когда просто не уложился в отведённое время: письма перенесены,
 * ящик исправен, следующий заход продолжит с курсора. Первая
 * синхронизация большого ящика в предел не укладывается НИКОГДА, то есть
 * человек, подключивший рабочий ящик, видел красное «перенос остановлен:
 * продолжить можно тем же заданием» — фразу из раздела миграции — и шёл
 * чинить то, что работает. Ровно то, против чего написан комментарий в
 * collector.ts; исправление дошло до итога сбора и остановилось перед
 * экраном.
 *
 * Теперь красное остаётся только там, где письма ДЕЙСТВИТЕЛЬНО не
 * перенеслись, а «не успели» — это 'ok' плюс строка `note`.
 *
 * 'never' (ни разу не запускался) — это не ошибка, а пустой lastSyncAt,
 * по нему интерфейс и отличает «ещё не собирали».
 */
export function toWebStatus(status: string, failed = 0): WebCollectorStatus {
  switch (status) {
    case 'running':
      return 'syncing';
    case 'error':
