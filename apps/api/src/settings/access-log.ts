 ...
 *
 * Страну по адресу здесь не определяем: чужих служб не спрашиваем (это
 * была бы выдача адресов наших пользователей наружу), а база GeoIP —
 * сотни мегабайт, которые надо ещё и обновлять. Если администратор такую
 * базу всё же положил и включил `GEOIP_LOGIN_POLICY`, страна дописывается
 * в запись о входе там же, где она рождается (routes/auth.ts), — но вид
 * адреса ниже остаётся в любом случае: он не зависит ни от какой базы.
 *
