...
/**
 * Адреса самого сервера приложения.
 *
 ...
 * время его жизни не меняются, а вызывается это на каждую строку журнала.
 */
...
export function isPrivateIp(ip: string): boolean {
  ...
  // IPv6: уникальные локальные (fc00::/7) и ссылочно-локальные (fe80::/10).
 ...
 */
export function markService(event: AccessEvent): AccessEvent {
  if (event.origin === 'app') return event;
  const ip = normalizeIp(event.ip);
  if (ip === '' || !ownAddresses().has(ip)) return event;
  /*
