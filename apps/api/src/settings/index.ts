      ...
      try {
        ...
        if (access) {
          ...
          const sweep = () => {
            ...
            /*
             * Закрытые записи о восстановлении и выгрузках — тем же
             * проходом.
             *
             * Раньше их не убирал никто: закрытие меняло состояние, а
             * строка оставалась навсегда. Самая объёмная —
             * trash_recovery_items: строка на КАЖДОЕ окончательно
             * удалённое письмо, с темой, отправителем и размером. Сто
             * человек по полсотни удалений в день — около 1,8 миллиона
             * строк в год. Пухнет и база, и каждая резервная копия, а
             * темы писем, выброшенных три года назад, продолжают лежать
             * и разъезжаться по копиям — при том что человек считает их
             * удалёнными.
             *
             * Срок берём тот же, что у истории входов: он ровно про то
             * же — сколько ещё осмысленно помнить о прошлом ящика.
             * Живое восстановление и готовый архив не трогаются: убираются
             * только закрытые записи.
             */
            void ownerDb
              .purgeClosedHistory(edge)
              .then((removed) => {
                if (removed > 0) {
                  logger.info({ removed }, 'Убраны закрытые записи восстановления и выгрузок');
                }
              })
              .catch((err: unknown) =>
                logger.warn(errorInfo(err), 'Уборка закрытых записей не удалась'),
              );
          };
