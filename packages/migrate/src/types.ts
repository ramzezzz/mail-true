...
export interface FolderReport {
  ...
  failed: number;
  /**
   * Письма, которых исходный сервер не отдал.
   *
   * ------------------------------------------------------------------
   * ЗАЧЕМ ОТДЕЛЬНЫЙ СЧЁТЧИК
   * ------------------------------------------------------------------
   * Поиск вернул UID, а FETCH по этому UID не вернул ничего. Обычно это
   * значит, что письмо удалили на источнике между двумя командами —
   * тогда всё в порядке. Но точно так же выглядит и сервер, который
   * письмо просто не отдал, а такое письмо не переедет НИКОГДА: курсор
   * через него проходит, и следующий запуск его уже не увидит.
   *
   * Раньше эти письма не попадали ни в один счётчик: `total` брался из
   * числа писем в папке, а `copied + skipped + failed` до него не
   * дотягивал — и заметить расхождение было нечем. Отдельным числом это
   * видно сразу, а «ошибкой» не называется: в самом частом случае
   * ошибки и нет.
   */
  missing: number;
  errors: string[];
...
export interface MailboxReport {
  ...
  skipped: number;
  failed: number;
  /** Писем, которых исходный сервер не отдал (см. FolderReport.missing). */
  missing: number;
