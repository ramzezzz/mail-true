...
/**
 * Самое короткое слово, которое вообще попадает в полнотекстовый индекс.
 *
 * Ровно то же число стоит в настройке Dovecot `fts_xapian = partial=3`
 * (infra/dovecot/conf/dovecot.conf.template): индексируются приставки слов
 * длиной от трёх букв, а всё, что короче, в индекс не попадает никогда.
 */
export const FTS_MIN_TERM = 3;

/** Что Xapian считает разделителем внутри слова. */
const FTS_WORD_SEPARATOR = /[^\p{L}\p{N}]+/u;

/**
 * Убрать из полнотекстового запроса обломки, которых в индексе нет.
 *
 * ЗАЧЕМ. Тема «Договор № 452/26: правки», скопированная в поиск, не
 * находила своего же письма. Причина — не в наших операторах (их тогда ещё
 * не было), а в том, как поисковый движок делит слова:
 *
 *   • пробел — граница слов для самого Dovecot, и слишком короткие слова он
 *     из запроса просто выбрасывает: «Договор 26» ищется как «Договор»;
 *   • знаки внутри слова (`/`, `-`, `.`, `_`) делит уже Xapian, и вот
 *     обломки он НЕ выбрасывает, а требует наравне с остальными. «452/26»
 *     превращается в «452 И 26», а слова «26» в индексе нет и быть не
 *     может — оно короче `partial=3`. Условие невыполнимо, выдача пуста.
 *
 * Всё письмо целиком при этом проиндексировано правильно: «Договор 452
 * правки» его находит. То есть поиск ломала не тема письма, а невыполнимое
 * условие, которое мы сами дописывали к запросу.
 *
 * ЧТО ДЕЛАЕТСЯ. У слова, разбитого знаком препинания, выбрасываются те
 * обломки, которых в индексе быть не может, и остаются те, что могут:
 * «452/26:» → «452». Тронуты только такие слова:
 *
 *   • слово без разделителей внутри остаётся как есть — «щщщ26» ищется
 *     целиком и находится;
 *   • слово, от которого не остаётся НИ ОДНОГО годного обломка («№»,
 *     «14:30», «из-за»), тоже остаётся как есть: выбросить его значило бы
 *     молча расширить запрос, а нам нужно сузить его до выполнимого, а не
 *     подменить другим.
 *
 * Запрос от этого становится шире (письма с «452» покажутся тоже), и это
 * осознанный размен: человек, вставивший тему письма в поиск, обязан это
 * письмо увидеть. Пустая выдача на верно набранный запрос — худший из
 * возможных ответов, потому что она выглядит как «письма нет».
 */
export function ftsSafeText(text: string): string {
  const words = text.split(/\s+/u).filter((w) => w !== '');
  const fixed = words.map((word) => {
    const parts = word.split(FTS_WORD_SEPARATOR).filter((p) => p !== '');
    // Разделителя внутри нет — делить нечего, слово уходит как есть.
    if (parts.length < 2) return word;
    const usable = parts.filter((p) => p.length >= FTS_MIN_TERM);
    if (usable.length === 0 || usable.length === parts.length) return word;
    return usable.join(' ');
  });
  return fixed.join(' ');
}
