...
interface RunParams<S extends z.ZodTypeAny> {
   ...
   * маленький потолок обрывает модели с «размышлением» до того, как они
   * дойдут до полезного ответа. Перевод письма в 256 токенов не влезет
   * никогда — получится обрубок на полуслове, за который ещё и заплачено.
   *
   * Настройку «Длина ответа, токенов» из панели она ПЕРЕКРЫВАЕТ снизу:
   * итог — max(настройка, эта граница). Молчать об этом нельзя, поэтому
   * в панели рядом с полем прямо названы возможности со своим минимумом
   * и их числа (apps/admin: раздел «Помощник»).
   */
    ...
    if (ctxUsesCache(params.ctx)) {
      ...
      if (cached !== null) {
        ...
        if (restored.ok) {
          ...
          /*
           * ОПИСЬ ОТДАЁТСЯ И ЗДЕСЬ, ХОТЯ НАРУЖУ НИЧЕГО НЕ УШЛО.
           *
           * Опись описывает не сетевой вызов, а ТЕКСТ: что из письма
           * попало в работу и что из него вырезано — цитаты, подпись,
           * хвост длиннее предела. Раньше на ответе из памяти здесь
           * стоял null, и вместе с ним пропадала строка «переведено без
           * таких-то частей»: первое нажатие «Перевести» честно
           * перечисляло вырезанное, а назавтра то же письмо (ответ живёт
           * в памяти 30 суток) переводилось молча. Перевод при этом
           * заменяет письмо на экране — то есть вырезанные куски просто
           * исчезали из виду.
           *
           * Что наружу ничего не уходило, видно по `cached: true`, и
           * интерфейс говорит это отдельной строкой.
           */
