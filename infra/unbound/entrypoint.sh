...
# ------------------------------------------------------------------
# Кому отвечать: своя docker-сеть
# ------------------------------------------------------------------
# В unbound.conf перечислены только приватные диапазоны RFC1918, и это
# работает ровно до тех пор, пока DOCKER_SUBNET лежит внутри них. Подсеть
# стека при этом настраивается (infra/.env), а Docker разрешает любую:
# стоит задать что-то за пределами 10/8, 172.16/12 и 192.168/16 — и unbound
# откажет всем службам сразу. Внешний DNS через него резолвят и postfix, и
# rspamd, то есть приём и отправка встают целиком, а сам unbound остаётся
# здоровым (его проверка спрашивает только себя) — искать причину пришлось
# бы по молчанию почты, а не по красной отметке.
if [ -n "${DOCKER_SUBNET}" ]; then
    cat > /etc/unbound/conf.d/15-access.conf <<EOF
server:
    access-control: ${DOCKER_SUBNET} allow
EOF
    echo "unbound: отвечаем своей сети ${DOCKER_SUBNET}"
else
    : > /etc/unbound/conf.d/15-access.conf
fi
