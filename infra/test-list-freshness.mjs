...
await run(
  'docker',
  ['compose', '-f', 'infra/docker-compose.yml', 'exec', '-T', 'postfix',
   'swaks', '--server', 'postfix:25', '--helo', 'client.example.com',
   '--from', 'freshness@example.com', '--to', MAIL,
   '--header', `Subject: ${MARK}`,
   '--header', 'Content-Type: text/plain; charset=UTF-8',
   '--body', 'проверка свежести списка'],
  { cwd: ROOT },
);
const sent = Date.now();
