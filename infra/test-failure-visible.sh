...
# где наоборот.
# Берём тот интерпретатор, который РАБОТАЕТ, а не тот, что нашёлся в PATH.
# На Windows (а разработка идёт там) `python3` — это заглушка Microsoft
# Store: файл существует, command -v его находит, а на запуск он открывает
# магазин приложений и ничего не считает. Скрипт выбирал её, разбор сводки
# молча возвращал пустоту, и три проверки ниже обвиняли админку в том, что
# она показывает остановленный антиспам исправным. Проверка «а посчитай
# мне единицу» отсеивает такую подмену.
PY=""
for candidate in python3 python; do
    path="$(command -v "$candidate" 2>/dev/null)" || continue
    [ -n "$path" ] || continue
    if [ "$("$path" -c 'print(1)' 2>/dev/null)" = "1" ]; then
        PY="$path"
        break
    fi
done
