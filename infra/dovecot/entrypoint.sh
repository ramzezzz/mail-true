...
(
    while true; do
        sleep 60
        # ------------------------------------------------------------------
        # ПРАВА ПРОВЕРЯЮТСЯ НА КАЖДОМ ОБХОДЕ, А НЕ ТОЛЬКО ПОСЛЕ ПРОВОРОТА
        # ------------------------------------------------------------------
        # Первый файл создаёт entrypoint и ставит 644 (выше). Дальше файл
        # заводит сам Dovecot — от root и с маской 077, то есть 0600
        # root:root, — и читать его серверу приложения (uid 5000) нечем:
        # «Журналы почты» в панели и история входов владельца ящика по
        # IMAP/POP3 молча пустеют.
        #
        # Поправить права сразу после `doveadm log reopen` НЕДОСТАТОЧНО, и
        # это выяснилось живой проверкой: команда возвращает управление
        # немедленно, а процессы Dovecot переоткрывают журнал асинхронно —
        # файл создаётся уже ПОСЛЕ нашего chmod, и права снова 600.
        #
        # Поэтому проверяем на каждом обходе — раз в минуту, ценой одного
        # stat. Так же устроено у Postfix (fix_maillog_permissions
        # вызывается регулярно, а не однажды).
        MODE=$(stat -c %a "$DOVELOG" 2>/dev/null || echo 644)
        [ "$MODE" = "644" ] || chmod 644 "$DOVELOG" 2>/dev/null || true
