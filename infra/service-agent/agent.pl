...
my $PROJECT_DIR = '';
my $OWN_ID      = '';
# Свой образ: в нём есть git и docker, и он заведомо есть на машине.
my $OWN_IMAGE   = '';
my $USE_PROD    = 0;
my $STARTUP_ERROR = '';
...
#
# Проверка идёт по 80-му порту ЧЕРЕЗ РАБОТАЮЩИЙ NGINX (--webroot -w
# /var/www/acme, общий том acme-challenge), поэтому останавливать ничего
# не нужно: ни nginx, ни почту. Разбор — в самой do_certbot и в
# infra/nginx/templates/app.conf.template.
#
# Здесь стояло описание другого способа — «--standalone, поэтому nginx на
# это время останавливается». Оно осталось от прежней версии и описывало
# обратное тому, что делает код: сопровождающий, поверив ему, убрал бы
# том acme-challenge как лишний и молча сломал выпуск сертификата из
# панели. Запускаем всё равно ЗДЕСЬ, а не в сервере приложения: у того
# нет и не будет доступа к Docker.
#
...
sub do_version {
    ...
    #
    # git запускается В ОДНОРАЗОВОМ КОНТЕЙНЕРЕ, а не здесь.
    #
    # $PROJECT_DIR — это путь ГЛАЗАМИ ДЕМОНА Docker, то есть путь на
    # хосте. Внутри контейнера посредника такого каталога нет: ему
    # смонтированы только отдельные файлы настроек, без .git. Прямой
    # вызов `git -C $PROJECT_DIR` поэтому молча возвращал пустоту — и
    # страница показывала пустую версию при живых образах служб. Найдено
    # живой проверкой на сервере.
    #
    # Образ берём свой же: он заведомо есть на машине, и git в нём есть
    # (ставится в Dockerfile). Каталог монтируется ТОЛЬКО ДЛЯ ЧТЕНИЯ —
...
#
#
# Значение одного ключа из infra/.env.
#
# Полноценный разбор не нужен и вреден: файл ИСПОЛНЯЕТСЯ установщиком, а
# здесь достаточно прочитать признак вида KEY=value. Отсутствие файла или
# ключа — пустая строка, а не отказ: вызывающий сам решает, что это
# значит.
#
sub env_value {
    my ($key) = @_;
...
sub compose_argv_host {
    ...
    # ПРОФИЛЬ АНТИВИРУСА.
    #
    # Служба clamav живёт под профилем, и без --profile compose её просто
    # не видит: «Обновить базовые образы» её образ не тянул, хотя и
    # страница, и документация называют clamav поимённо. Именно ради
    # исправлений безопасности в антивирусе эту кнопку и нажимают.
    #
    # Заодно это убирает предупреждение «осиротевший контейнер» у
    # работающего clamav при `up -d`.
    #
    # Установщик делает ровно так же (install/lib/common.sh, compose_args).
    push @argv, '--profile', 'clamav' if env_value('CLAMAV_ENABLED') eq 'true';
    return @argv;
}
...
sub do_update {
    ...
    if ($mode eq 'code') {
        ...
        #
        # МИГРАЦИИ — ДО СБОРКИ И ПОДЪЁМА.
        #
        # Их здесь не было вовсе: обновление переводило код вперёд и
        # поднимало новый сервер приложения поверх СТАРОЙ схемы —
        # насовсем. Postgres выполняет каталог миграций сам только при
        # создании пустого тома, а apply_migrations вызывался лишь из
        # install.sh. Раздел панели потом честно показывал красное «Не
        # применено миграций», а этот экран рапортовал зелёным «готово».
        #
        # Порядок тот же, что в консольном установщике: сначала схема,
        # потом новый код. Отказ применения останавливает обновление
        # (set -e) ДО сборки — то есть работающий сервер остаётся на
        # прежнем коде и прежней схеме, а не превращается в новый код
        # поверх старой базы.
        #
        $script = join "\n",
            'set -e',
            ...
            'bash ' . sh_quote("$root/install/apply-migrations.sh") . ' update',
    ...
    if ($rc != 0) {
        my $detail = trim(($err || $out) || "код возврата $rc");
        #
        # Два нажатия подряд с двух вкладок: обе проверки «уже идёт?»
        # успевают ответить «нет», и второй `docker run` упирается в
        # занятое имя. Это то же самое «обновление уже идёт», и человеку
        # надо сказать именно это, а не показать сообщение демона про
        # конфликт имён контейнера.
        #
        if ($detail =~ /already in use/i) {
            return reply($client, 409, { error => 'обновление уже идёт' });
        }
...
sub do_certbot {
    ...
    # ------------------------------------------------------------------
    # СНАЧАЛА — А ЗНАЕМ ЛИ МЫ, ГДЕ ЖИВЁТ УСТАНОВКА
    # ------------------------------------------------------------------
    # $STARTUP_ERROR спрашивают все остальные действия (check_service), а
    # выпуск сертификата — нет. Между тем при неудачном опознании каталога
    # проекта $PROJECT_DIR остаётся пустым, и монтирование ниже
    # превращается из «<проект>/data/certs» в «/data/certs».
    #
    # Ошибка при этом ТИХАЯ, и в этом вся беда: демон заводит на хосте
    # пустой /data/certs, копирование внутрь одноразового контейнера
    # проходит успешно, код возврата ноль — и посредник отвечает «готово,
    # сертификат разложен». Настоящий infra/data/certs не тронут, nginx,
    # Postfix и Dovecot продолжают жить на самоподписанном, а панель
    # показывает успех.
    #
    # Соседние действия ошибаются громко: пустой каталог уходит в compose
    # как «--project-directory ''», и тот падает. Тихо ошибался только
    # выпуск — он единственный собирает «docker run» руками.
    if ($STARTUP_ERROR ne '') {
        return reply($client, 503, { error => $STARTUP_ERROR });
    }
    ...
    #
    # Пробный выпуск ВСЕГДА выпускает заново.
    #
    # `--keep-until-expiring` бережёт попытки настоящего Let's Encrypt
    # (их пять в час на домен): если набор имён тот же и срок ещё не
    # подошёл, certbot ничего не делает и выходит с кодом 0.
    #
    # Для пробного выпуска это ровно та беда, ради которой его и
    # нажимают. Человек починил DNS, нажал «Проверить пробным выпуском»,
    # получил зелёное «домен подтверждается, порт 80 доступен снаружи» —
    # и снял флажок. Настоящий выпуск падает, попытка из пяти потрачена.
    # А проверки не было вовсе: certbot просто увидел свежий
    # испытательный сертификат и вышел.
    #
    # У испытательного центра пределы высокие, поэтому здесь заставляем
    # выпускать заново: только так проверка домена действительно
    # выполняется.
    if ($staging) {
        push @args, '--staging';
        push @args, '--force-renewal';
    }
    ...
    unless ($staging) {
        my @copy = (
            ...
            "set -e; cp -L /le/live/$cert_name/fullchain.pem /certs/mail.crt.tmp; "
              ...
              # перечитывает пару, только когда она устоялась (два
              # одинаковых замера подряд), и потому не подхватит
