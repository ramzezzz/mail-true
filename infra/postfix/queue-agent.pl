...
# ------------------------------------------------------------------
# ПРЕДЕЛ ЧИСЛА СТРОК ОЧЕРЕДИ.
#
# `postqueue -j` печатает по объекту JSON на письмо, и печатает ВСЮ
# очередь. На застрявшем сервере в ней бывают сотни тысяч писем: это
# десятки мегабайт вывода, которые прежде целиком попадали в память
# этого посредника, целиком уезжали по сети и целиком разбирались
# сервером приложения — при потолке кучи в 512 МБ он от такого ответа
# просто умирал, и панель переставала работать ровно тогда, когда в неё
# и заходят.
#
# Поэтому чтение идёт построчно и обрывается на пределе: дальше вывод не
# нужен никому. Сервер приложения показывает не больше двадцати тысяч
# писем, здесь предел с запасом — и он же спасает от переполнения
# памяти самого посредника.
# ------------------------------------------------------------------
my $MAX_QUEUE_LINES = $ENV{QUEUE_AGENT_MAX_LINES} // 20000;
my $CMD_TIMEOUT       = 20;
...
sub queue_list {
    my ($client) = @_;
    my ($rc, $lines, $truncated, $err) = run_lines($MAX_QUEUE_LINES, '/usr/sbin/postqueue', '-j');
    if ($rc != 0) {
    ...
    return reply($client, 200, {
        ok        => \1,
        lines     => $lines,
        # Показанное неполно: очередь длиннее предела. Разбирающая
        # сторона обязана сказать об этом человеку, иначе он решит, что
        # видит всю очередь.
...
# ------------------------------------------------------------------
# Запуск программы с ПОСТРОЧНЫМ чтением и пределом числа строк.
#
# Отличие от run() одно, но важное: вывод не копится в памяти целиком.
# Дочитав предел, мы закрываем канал и убиваем потомка — postqueue на
# этом останавливается сам (пишет в закрытый канал).
# ------------------------------------------------------------------
sub run_lines {
    my ($limit, @argv) = @_;
    my $errfile = "/tmp/queue-agent.$$.err";
    # ------------------------------------------------------------------
    # ОБЪЯВЛЯЕМ ПО ОДНОЙ, А НЕ СПИСКОМ.
    #
    # Здесь стояло `my (@lines, $err, $rc, $truncated) = ((), '', -1, 0)`.
    # Массив в списочном присваивании забирает ВСЁ, что справа: @lines
    # становился ('', -1, 0), а три скаляра — пустыми. Дальше `$rc != 0`
    # для пустого значения равнялось `0 != 0`, то есть лжи: любой отказ
    # (сработавший будильник на 20 секунд, неудачный порождённый процесс)
    # уходил наверх как «200 ОК, очередь прочитана», а тремя мусорными
    # строками занимал места в пределе.
    #
    # Цена этого была наибольшей ровно в аварии: застрявший postqueue не
    # отвечает, панель показывает «Очередь пуста: все принятые письма
    # доставлены», порог тревоги по глубине очереди не срабатывает, и
    # ноль уезжает в постоянный снимок показателей — навсегда.
    # ------------------------------------------------------------------
    my @lines;
    my $err = '';
    my $rc = -1;
    ...
    eval {
        ...
        die "не удалось породить процесс: $!\n" unless defined $pid;
        if ($pid == 0) {
            # Расположение сигналов переживает exec, а у посредника
            # SIGPIPE намеренно игнорируется (см. начало файла). Потомку
            # это вредно: дочитав предел, мы закрываем канал и ждём, что
            # программа на записи в него закончится сама. С игнором она
            # вместо этого получает ошибку ввода-вывода и продолжает
            # обходить очередь дальше — на трёхсоттысячной очереди это
            # десятки секунд, за которые вызывающая сторона успевает
            # отвалиться по своему пределу ожидания.
            $SIG{PIPE} = 'DEFAULT';
            open(STDERR, '>', $errfile);
            exec { $argv[0] } @argv;
            exit 127;
        ...
        # Порядок здесь важен: сперва сигнал, потом закрытие.
        #
        # `close` на канале ждёт завершения потомка (внутри waitpid), и
        # страховочный kill после него не сокращал ожидание, а приходил
        # уже по пожатому номеру процесса — то есть в лучшем случае был
        # бесполезен, а в худшем достался бы постороннему процессу с тем
        # же номером.
        kill 'TERM', $pid if $truncated;
        close $fh;
        # Обрыв по пределу — не ошибка: код возврата у оборванной
        # программы ненулевой, и без этой поправки честно прочитанная
        # очередь превратилась бы в «postqueue: ошибка».
        $rc = $truncated ? 0 : $? >> 8;
        alarm 0;
        1;
    } or do {
