# Дизайн-система интерфейса (снято с эталонного интерфейса)

Источник: живой интерфейс эталонный интерфейс, снято 2026-08-05.

В репозитории лежат два файла — `brand/reference/`:
...
Первый из них читает сборка: `apps/web/scripts/build-tokens.mjs` отбирает
из него токены в `apps/web/src/styles/tokens.css`, и оба образа
веб-интерфейса копируют его внутрь при сборке. Поэтому он и живёт в
репозитории, а не рядом с остальными выгрузками.

Остальное — снимки экрана, геометрия строк, дампы DOM — в репозиторий не
попадает: это выгрузки с живого ящика, в них адреса и переписка. Их место
на машине разработчика, и `.gitignore` следит за этим.
