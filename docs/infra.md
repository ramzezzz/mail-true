...
- Белые/чёрные списки антиспама — том `mailtrue_rspamd-maps`
  (`/etc/rspamd/maps.d` в контейнере). Их правит админка через `/savemap`,
  rspamd перечитывает карты сам (10 секунд). Файлы `infra/rspamd/maps.d/*.map`
  — это ЗАГОТОВКИ: они подключены только на чтение (`/etc/rspamd/maps.seed`),
  и entrypoint копирует из них в том лишь недостающие файлы. Так сделано
  потому, что раньше продукт правил файлы прямо в рабочем дереве git, и
  документированное обновление `git pull` обрывалось о собственные правки
  продукта, а привычное «лечение» стирало списки администратора.
  Списки входят в резервную копию (том `rspamd-maps`, см. `install/backup.sh`).
